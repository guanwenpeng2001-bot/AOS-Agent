...
- Never run the full vitest suite directly: it includes e2e tests that activate when endpoint/auth env vars are present. For all non-e2e tests, run `./test.sh` from the repo root. Otherwise run specific tests from the package root:
  - Vitest: `node "$(git rev-parse --show-toplevel)/node_modules/vitest/dist/cli.js" --run <path-to-test>`
  - `packages/tui` (`node:test`): `node --test <path-to-test>`
- If you create or modify a test file, run it and iterate on test or implementation until it passes.
- For `packages/coding-agent/test/suite/`, use `test/suite/harness.ts` + the fake provider. No real provider APIs, keys, or paid tokens.
...
- Pre-commit blocks lockfile commits unless `AOS_AGENT_ALLOW_LOCKFILE_CHANGE=1`. Don't bypass unless the user wants the lockfile change committed.

## Git

Multiple AOS Agent sessions may be running in this cwd at the same time, each modifying different files. Git operations that touch unstaged, staged, or untracked files outside your own changes will stomp on other sessions' work. Follow these rules:
...
- Generated model catalog output under `packages/ai/src/` may always be included alongside your files.
...
See `CONTRIBUTING.md` for owner/maintainer review and the issue quality bar.
...
## Testing AOS Agent Interactive Mode with tmux

Run the TUI in a controlled terminal (from the repo root):

```bash
tmux new-session -d -s aos-test -x 80 -y 24
tmux send-keys -t aos-test "./aos-test.sh" Enter
sleep 3 && tmux capture-pane -t aos-test -p     # capture after startup
tmux send-keys -t aos-test "your prompt here" Enter
tmux send-keys -t aos-test Escape               # special keys (also C-o for ctrl+o, etc.)
tmux kill-session -t aos-test
```
...
2. **Local smoke test**: build an unpublished release and smoke test from outside the repo (so it can't resolve workspace files):
   ```bash
   npm run release:local -- --out /tmp/aos-local-release --force
   cd /tmp

   # Node package install smoke tests
   /tmp/aos-local-release/node/aos --help
   /tmp/aos-local-release/node/aos --version
   /tmp/aos-local-release/node/aos --list-models
   /tmp/aos-local-release/node/aos -p "Say exactly: ok"
   /tmp/aos-local-release/node/aos

   # Bun binary smoke tests
   /tmp/aos-local-release/bun/aos --help
   /tmp/aos-local-release/bun/aos --version
   /tmp/aos-local-release/bun/aos --list-models
   /tmp/aos-local-release/bun/aos -p "Say exactly: ok"
   /tmp/aos-local-release/bun/aos
   ```
   Verify both Node and Bun startup, model/account listing, interactive startup, and at least one real prompt with the intended default provider. The bare commands `/tmp/aos-local-release/node/aos` and `/tmp/aos-local-release/bun/aos` start interactive mode; run each in tmux, submit a prompt, and wait for the model reply before considering the interactive smoke test passed. Failures are release blockers unless the user explicitly accepts the risk.

3. **Run the release script**:
   ```bash
   AOS_AGENT_ALLOW_LOCKFILE_CHANGE=1 npm_config_min_release_age=0 npm run release:patch    # fixes + additions
   AOS_AGENT_ALLOW_LOCKFILE_CHANGE=1 npm_config_min_release_age=0 npm run release:minor    # breaking changes
   ```
