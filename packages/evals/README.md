...
AOS Agent evals are behavioral, model-backed checks for AOS Agent workflows. They adapt a real `AgentSession` to `vitest-evals`, run
it in isolated temporary project and agent directories, and attach native AOS Agent session artifacts.
...
```bash
AOS_AGENT_PROVIDER=openai AOS_AGENT_MODEL=gpt-5.6-sol npm run eval
```
...
Authentication comes from AOS Agent's normal `ModelRuntime`, including subscription credentials and provider API-key
...
native AOS Agent session JSONL attachments under `sessions/`. These files may contain prompts, responses, source code, and tool
output.

## Writing evals

Follow [`vitest-evals`](https://github.com/getsentry/vitest-evals) for general suite, judge, assertion, and normalized
trace guidance. AOS Agent-specific evals use `createAosCodingAgentHarness(...)` from `src/aos-harness.ts`, with one harness bound
...
const harness = createAosCodingAgentHarness({ noTools: "all" });

describeEval("AOS Agent smoke", { harness }, (it) => {
...
### Configuring the AOS Agent harness

`createAosCodingAgentHarness(...)` accepts:
...
- `noTools`: AOS Agent's tool-disable configuration.
...
```ts
const harness = createAosCodingAgentHarness({
...
prompt creates or changes AOS Agent resources:

```ts
const result = await run([
	{ type: "prompt", content: "Create an AOS Agent extension." },
	{ type: "reload" },
...
Use `output` to expose scenario-specific, JSON-safe behavior without adding that behavior to the generic AOS Agent adapter:
...
Harnesses may differ by prompt, tools, skills, model, or any other AOS Agent configuration:
...
The AOS Agent harness snapshots native session JSONL before deleting its temporary workspace. An eval-only `afterEach` hook
