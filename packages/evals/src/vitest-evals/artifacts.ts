...
export const AOS_AGENT_SESSION_SNAPSHOT_ARTIFACT = "aosAgentSessionJsonl";

const evalSessionArtifactKey = Symbol("aos-agent-evals-session-artifact");
const evalSourceArtifactKey = Symbol("aos-agent-evals-source-artifact");

interface AosAgentSessionAttachment extends TestAttachment {
	name: "session.jsonl";
...
interface AosAgentSessionArtifact extends TestArtifactBase {
	type: "@aos-agent/evals:session";
	runId: string;
	attachments: [AosAgentSessionAttachment] | [];
}

interface SourceArtifact extends TestArtifactBase {
	type: "@aos-agent/evals:source";
	runId: string;
...
declare module "vitest" {
	interface TestArtifactRegistry {
		[evalSessionArtifactKey]: AosAgentSessionArtifact;
...
export async function recordEvalSessionArtifact(
	...
	const session = run.artifacts?.[AOS_AGENT_SESSION_SNAPSHOT_ARTIFACT];
	if (session === undefined) return;
	if (typeof runId !== "string" || typeof session !== "string") {
		throw new TypeError("AOS Agent eval session artifact metadata is invalid.");
	}
...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@aos-agent/evals:session" &&
				artifact.type !== "@aos-agent/evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@aos-agent/evals:session" ? "sessions" : "sources";
