...
import { AOS_AGENT_SESSION_SNAPSHOT_ARTIFACT, persistEvalArtifactReferences } from "./artifacts.ts";
...
async function appendHarnessRunReport(test: TestCase): Promise<void> {
	const artifactDirectory = process.env.AOS_AGENT_EVAL_ARTIFACT_DIR?.trim();
	...
	const metadata = Object.fromEntries(
		Object.entries(run.artifacts ?? {}).filter(([name]) => name !== "runId" && name !== AOS_AGENT_SESSION_SNAPSHOT_ARTIFACT),
	);
	const record = {
		schemaVersion: 1,
		runId,
