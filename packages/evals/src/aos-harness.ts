...
import { AOS_AGENT_SESSION_SNAPSHOT_ARTIFACT } from "./vitest-evals/artifacts.ts";

export type AosCodingAgentInput = string | Array<{ type: "prompt"; content: string } | { type: "reload" }>;

type AosCodingAgentModelSelection = {
	provider: string;
	id: string;
};

type AosCodingAgentHarnessOptions = {
	name?: string;
	model?: AosCodingAgentModelSelection;
	noTools?: CreateAgentSessionOptions["noTools"];
	transformSystemPrompt?: (defaultPrompt: string) => string;
};

type AosCodingAgentHarnessWithOutput<TOutput extends JsonValue> = AosCodingAgentHarnessOptions & {
	output: (args: { response: string; session: AgentSession }) => TOutput | Promise<TOutput>;
};

export function resolveModelSelection(
	explicitModel: AosCodingAgentModelSelection | undefined,
	environment: { AOS_AGENT_PROVIDER?: string; AOS_AGENT_MODEL?: string } = process.env,
): AosCodingAgentModelSelection {
	const provider = (explicitModel?.provider ?? environment.AOS_AGENT_PROVIDER)?.trim();
	const id = (explicitModel?.id ?? environment.AOS_AGENT_MODEL)?.trim();
	if (!provider || !id) {
		throw new Error("Select a harness model explicitly or set both AOS_AGENT_PROVIDER and AOS_AGENT_MODEL as defaults.");
	}
	return { provider, id };
}
...
async function runAosCodingAgent<TOutput extends JsonValue>(
	input: AosCodingAgentInput,
	signal: AbortSignal | undefined,
	setArtifact: HarnessContext["setArtifact"],
	options: AosCodingAgentHarnessOptions | AosCodingAgentHarnessWithOutput<TOutput>,
	...
	const root = await mkdtemp(join(tmpdir(), "aos-agent-eval-"));
	...
	try {
		...
		try {
			...
			if (response === undefined) throw new Error("AOS Agent eval input must include at least one prompt step.");
...
export function createAosCodingAgentHarness<TOutput extends JsonValue>(
	options: AosCodingAgentHarnessWithOutput<TOutput>,
): Harness<AosCodingAgentInput, TOutput>;
export function createAosCodingAgentHarness(options?: AosCodingAgentHarnessOptions): Harness<AosCodingAgentInput, string>;
export function createAosCodingAgentHarness<TOutput extends JsonValue>(
	options: AosCodingAgentHarnessOptions | AosCodingAgentHarnessWithOutput<TOutput> = {},
) {
	return createHarness<AosCodingAgentInput, string | TOutput>({
		name: options.name ?? "aos-agent",
		run: ({ input, signal, setArtifact }) => runAosCodingAgent(input, signal, setArtifact, options),
	});
}
