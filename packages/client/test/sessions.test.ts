...
import { AosSessionDetachedError, AosSessionOwnershipError } from "../src/index.ts";
...
describe("AosClient", () => {
	test("keeps multiple session handles independent and enforces detach", async () => {
		...
		expect(second.attached).toBe(true);
		await expect(first.abort()).rejects.toBeInstanceOf(AosSessionDetachedError);
	});
	...
	test("rejects commands while releasing and restores an explicit detach after failure", async () => {
		...
		await expect(lease.abort()).rejects.toBeInstanceOf(AosSessionDetachedError);
