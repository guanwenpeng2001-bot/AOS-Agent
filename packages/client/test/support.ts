...
import type { ByteTransport, ByteTransportHandlers, AosSessionHandle } from "../src/index.ts";
import { AosClient } from "../src/index.ts";
...
export function createClient(server: MemoryByteServer): AosClient {
	return new AosClient({
		transportFactory: (handlers) => server.connect(handlers),
	});
}

export async function connectClient(server: MemoryByteServer): Promise<AosClient> {
