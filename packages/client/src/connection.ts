...
import { PiDisconnectedError, AosServerError, toDisconnectedError, toError } from "./errors.ts";
...
export class Connection {
	...
	constructor(options: ConnectionOptions) {
		...
		if (
			...
			throw new TypeError(`AosClient maxFrameLength must be an integer between 1 and ${MAX_UINT32}`);
		}
	}
	...
	connect(): Promise<ServerSnapshot> {
		if (this.#lifecycle.state !== "disconnected") {
			return Promise.reject(new PiDisconnectedError(`AosClient is already ${this.#lifecycle.state}`));
		}
	...
	#handleMessage(message: ServerMessage): void {
		...
		if (lifecycle.state === "connecting") {
			if (message.type === "hello_error") {
				this.#failAndClose(new AosServerError(message.error));
				return;
			}
