...
export class AosClient {
	readonly #options: AosClientOptions;
	...
	static async connect(options: AosClientOptions): Promise<AosClient> {
		const client = new AosClient(options);
		try {
	...
	connect(): Promise<ServerSnapshot> {
		if (this.#disposed) return Promise.reject(new AosClientDisposedError());
	...
	async createSession(options: CreateSessionOptions = {}): Promise<AosSessionHandle> {
	...
	async attachSession(sessionId: string): Promise<AosSessionHandle> {
		return this.acquireSession(sessionId, { mode: "shared" });
	}

	async acquireSession(sessionId: string, options: AcquireSessionOptions): Promise<AosSessionHandle> {
		this.#assertNotDisposed();
	...
	#createSessionLease(sessionId: string, token: SessionLeaseToken): AosSessionHandle {
		...
		const assertActive = () => {
			...
			if (!isActive()) throw new AosSessionDetachedError(sessionId);
		};
	...
	#handleMessage(message: ResponseEnvelope | EventEnvelope): void {
		...
		if (!message.ok) {
			pending.reject(new AosServerError(message.error));
			return;
		}
	...
	dispose(): Promise<void> {
		...
		const error = new AosClientDisposedError();
	...
	#assertNotDisposed(): void {
		if (this.#disposed) throw new AosClientDisposedError();
	}
	...
	#reserveSessionLease(sessionId: string, mode: SessionLeaseMode): SessionLeaseToken {
		...
		if (mode === "exclusive" && count > 0) {
			throw new AosSessionOwnershipError(sessionId, `Session ${sessionId} already has an active lease`);
		}
		if (mode === "shared" && this.#exclusiveSessionLeases.has(sessionId)) {
			throw new AosSessionOwnershipError(sessionId, `Session ${sessionId} has an exclusive lease`);
		}
