...
class UnixListener implements AosServerListener {
...
export function createUnixListener(options: UnixListenerOptions): AosServerListener {
...
function resolveUnixListenerOptions(options: UnixListenerOptions): ResolvedUnixListenerOptions {
	validateUnixSocketPath(options.path, "AosServer Unix socket path");
	const mode = options.mode ?? DEFAULT_SOCKET_MODE;
	if (!Number.isInteger(mode) || mode < 0 || mode > 0o777) {
		throw new TypeError("AosServer Unix socket mode must be an integer between 0 and 0o777");
	}
	...
	if (!Number.isSafeInteger(maxPendingBytes) || maxPendingBytes < maxFrameLength + 4) {
		throw new TypeError("AosServer maxPendingBytes must be a safe integer at least maxFrameLength + 4");
	}
	...
	if (
		...
		throw new TypeError(`AosServer gracefulCloseTimeoutMs must be an integer between 1 and ${MAX_TIMER_DELAY_MS}`);
	}
	return {
		path: options.path,
		mode,
