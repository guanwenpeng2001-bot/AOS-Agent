...
import type { Command, EventEnvelope, SessionMetadata, SessionSnapshot } from "@aos-agent/protocol";
import type { ByteConnection, ConnectionState } from "./connection.ts";
import { AosServerError } from "./errors.ts";
import type { CreateSessionOptions, AosServerService, AosSessionRuntime, AosSessionRuntimeEvent } from "./types.ts";

interface LiveSession {
	id: string;
	runtime: AosSessionRuntime;
...
export class LiveSessionManager {
	...
	private async acquire(id: string, acquireRuntime: () => Promise<AosSessionRuntime>): Promise<LiveSession> {
		for (;;) {
			const existing = this.liveSessions.get(id);
			if (existing) {
				if (existing.terminal) throw new AosServerError("session_locked", `Session runtime is terminating: ${id}`);
	...
	private async create(id: string, acquireRuntime: () => Promise<AosSessionRuntime>): Promise<LiveSession> {
		const runtime = await acquireRuntime();
		if (this.options.isClosing()) {
			await runtime.dispose();
			throw new Error("AosServer closed while acquiring a session runtime");
		}
		let live: LiveSession | undefined;
		try {
			const snapshot = await runtime.snapshot();
			if (snapshot.id !== id) {
				throw new AosServerError(
					"invalid_request",
	...
	private handleRuntimeEvent(live: LiveSession, event: AosSessionRuntimeEvent): void {
		if (event.type === "error") {
	...
	private async terminate(live: LiveSession, error: AosServerError): Promise<void> {
	...
	private async normalizedSnapshot(live: LiveSession): Promise<SessionSnapshot> {
		...
		if (snapshot.id !== live.id) {
			throw new AosServerError("invalid_request", `Runtime session ID changed from ${live.id} to ${snapshot.id}`);
		}
		return {
			...snapshot,
	...
	private async attach(connection: ConnectionState, live: LiveSession): Promise<void> {
		if (connection.disconnected || connection.stage !== "ready" || connection.connection.closed) {
			await this.maybeDispose(live);
			throw new AosServerError("invalid_request", "Connection closed while attaching to a session");
		}
	...
	private requireAttached(connection: ConnectionState, sessionId: string): LiveSession {
		if (!connection.sessionIds.has(sessionId)) {
			throw new AosServerError("invalid_request", `Connection is not attached to session ${sessionId}`);
		}
		const live = this.liveSessions.get(sessionId);
		if (!live || live.terminal || live.disposing) {
			throw new AosServerError("not_found", `Session is not live: ${sessionId}`);
		}
		return live;
	}
