...
export class TestSessionRuntime implements AosSessionRuntime {
	readonly disposed = new Deferred<void>();
	disposeCount = 0;
	readonly steers: PromptInput[] = [];
	private readonly stored: StoredSession;
	private readonly onDispose: () => void;
	private readonly listeners = new Set<(event: AosSessionRuntimeEvent) => void>();
	...
	async prompt(input: PromptInput): Promise<void> {
		if (this.getPhase() !== "idle") throw new AosServerError("busy", "A prompt is already running");
	...
	async steer(input: PromptInput): Promise<void> {
		if (this.getPhase() === "idle") throw new AosServerError("busy", "There is no active prompt to steer");
	...
	async abort(): Promise<void> {
		if (!this.pendingPrompt) throw new AosServerError("busy", "There is no active prompt to abort");
		this.pendingPrompt.done.resolve("aborted");
	}

	async setModel(model: ModelRef): Promise<void> {
		if (this.getPhase() !== "idle") throw new AosServerError("busy", "Session is busy");
		this.update({ model });
	}
	...
	emitError(error: AosServerError): void {
...
export class TestServerService implements AosServerService {
	...
	async createSession(options: CreateSessionOptions): Promise<AosSessionRuntime> {
		this.lastCreatedId = options.id;
		if (this.sessions.has(options.id)) throw new AosServerError("session_locked", "Session already exists");
		this.seed(options.id, options.name, options.cwd, options.model, options.thinkingLevel);
		return this.acquire(options.id);
	}

	async openSession(sessionId: string): Promise<AosSessionRuntime> {
		if (!this.sessions.has(sessionId)) throw new AosServerError("not_found", `Unknown session: ${sessionId}`);
		if (this.locked.has(sessionId)) throw new AosServerError("session_locked", `Session is locked: ${sessionId}`);
