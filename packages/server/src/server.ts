...
export class AosServer {
	readonly id: string;

	private readonly listeners: readonly AosServerListener[];
	...
	constructor(service: AosServerService, options: AosServerOptions) {
	...
	start(): Promise<this> {
		if (this.started) return Promise.reject(new Error("AosServer is already started"));
		if (this.startPromise) return Promise.reject(new Error("AosServer is already starting"));
		if (this.closing) return Promise.reject(new Error("AosServer is closing or closed"));
		this.startPromise = this.startInternal();
		return this.startPromise;
	}

	private async startInternal(): Promise<this> {
		const started: AosServerListener[] = [];
		try {
	...
	private toProtocolError(error: unknown): ProtocolError {
		...
		if (error instanceof AosServerError) {
...
function resolveOptions(options: AosServerOptions): { maxFrameLength: number; handshakeTimeoutMs: number } {
	if (!Array.isArray(options.listeners)) throw new TypeError("AosServer listeners must be an array");
	if (options.serverId === "") throw new TypeError("AosServer serverId must not be empty");
	const maxFrameLength = options.maxFrameLength ?? DEFAULT_MAX_FRAME_LENGTH;
	if (!Number.isSafeInteger(maxFrameLength) || maxFrameLength <= 0 || maxFrameLength > MAX_UINT32) {
		throw new TypeError(`AosServer maxFrameLength must be an integer between 1 and ${MAX_UINT32}`);
	}
	...
	if (
		...
		throw new TypeError(`AosServer handshakeTimeoutMs must be an integer between 1 and ${MAX_TIMER_DELAY_MS}`);
	}
