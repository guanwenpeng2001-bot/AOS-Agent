...
import { encodeClientMessage, encodeFrame, PROTOCOL_VERSION } from "@aos-agent/protocol";
import { afterEach, describe, expect, test } from "vitest";
import { InternalServerError, NotImplementedError, type AosServer, AosServerError } from "../src/index.ts";
...
): Promise<{ server: AosServer; service: TestServerService }> {
...
async function connect(server: AosServer): Promise<ProtocolTestClient> {
...
describe("Unix transport conformance", () => {
	...
	test("disconnects attached clients when a runtime reports a terminal error", async () => {
		...
		runtime.emitError(new AosServerError("session_locked", "lock ownership lost"));
		await client.waitForClose();
