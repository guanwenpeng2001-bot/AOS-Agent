...
import type { ModelMetadata, SessionSnapshot, TranscriptProgress } from "@aos-agent/protocol";
import { afterEach, describe, expect, test } from "vitest";
import type { CreateSessionOptions, AosServer, AosSessionRuntime } from "../src/index.ts";
import {
...
async function connect(server: AosServer): Promise<Client> {
...
describe("AosServer Unix integration", () => {
	...
	test("rejects and disposes a service runtime with the wrong server-assigned ID", async () => {
		class WrongIdService extends MemoryService {
			override async createSession(options: CreateSessionOptions): Promise<AosSessionRuntime> {
