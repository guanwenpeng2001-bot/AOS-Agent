...
import { ServerMessageDecoder } from "@aos-agent/protocol";
...
let server: AosServer | undefined;
...
test("requires explicit listeners", () => {
	expect(() => Reflect.construct(AosServer, [service, {}])).toThrow(/listeners/);
});
...
test("handshake timeout cleanup does not wait for a blocked output queue", async () => {
	...
	const core = new AosServer(service, {
		listeners: [],
		maxFrameLength: 1024,
...
test("rejects timeout values above Node's maximum timer delay", () => {
	const unix = { path: "/tmp/aos-server-timeout-test.sock" };
