...
function parseGitHubCopilotModelCatalog(raw: unknown, allowPolicyFallback: boolean) {
	...
	const accountModels = data.flatMap((rawItem) => {
		...
		return [{ id, pickerEnabled: item.model_picker_enabled === true, policyState: asRecord(item.policy)?.state }];
	});
	...
	const policyModelIds = accountModels
		.filter((model) => model.policyState === "unconfigured" && Object.hasOwn(GITHUB_COPILOT_MODELS, model.id) && (model.pickerEnabled || usePolicyFallback))
		.map((model) => model.id);
...
async function fetchWithRateLimitRetry(
	...
	init: RequestInit,
	signal: AbortSignal,
	policy: { maxRetries: number; maxElapsedMs: number },
): Promise<Response> {
	const budget = policy.maxRetries > 0 && policy.maxElapsedMs > 0 ? AbortSignal.timeout(policy.maxElapsedMs) : undefined;
	const requestSignal = budget ? AbortSignal.any([signal, budget]) : signal;
	const deadline = budget ? Date.now() + policy.maxElapsedMs : undefined;
	for (let retry = 0; ; retry++) {
		const response = await fetch(url, { ...init, signal: AbortSignal.any([requestSignal, AbortSignal.timeout(5000)]) });
		if (response.status !== 429 || retry === policy.maxRetries) return response;
		...
		if (deadline !== undefined && delayMs >= deadline - Date.now()) return response;
		await response.body?.cancel();
		if (delayMs > 0) await sleep(delayMs, requestSignal);
	}
}
...
async function enableGitHubCopilotModel(
	...
	if (response.status === 429) throw new Error(`${response.status} ${response.statusText}: ${await response.text()}`);
	return response.ok;
}
...
async function enableGitHubCopilotModels(
	...
	for (const modelId of modelIds) {
		try {
			if (await enableGitHubCopilotModel(token, modelId, enterpriseDomain, signal)) enabled.push(modelId);
		} catch (error) {
			if (signal.aborted) throw error;
			break;
		}
