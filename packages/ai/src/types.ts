import type { TelemetryContext } from "@aos-agent/telemetry";
import type { AnthropicOptions } from "./api/anthropic-messages.ts";
import type { AosMessagesOptions } from "./api/aos-messages.ts";
...
export interface StreamOptions extends ProviderRequestOptions<Model<Api>> {
	 ...
	 * SGLang, ...) receive parameters the common model contract does not model, e.g. `top_p`, `top_k`, `min_p`,
...
export interface ApiOptionsMap {
	...
	"aos-messages": AosMessagesOptions;
}
...
export interface OpenAICompletionsCompat {
	...
	/** Whether streamed responses include `finish_reason`. When false, AOS Agent infers `stop` or `toolUse` when the stream ends. Default: true. */
	...
	thinkingFormat?:
		...
		| "string-thinking"
		| "ant-ling";
	/** Kwargs to send as `chat_template_kwargs` when `thinkingFormat` is `chat-template`. Use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for AOS Agent-controlled thinking values. */
	chatTemplateKwargs?: Record<string, ChatTemplateKwargValue>;
	/** Arguments to send as `chat_template_args` when `thinkingFormat` is `baseten`. Use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for AOS Agent-controlled thinking values. */
...
export interface Model<TApi extends Api> {
	...
	reasoning: boolean;
	/**
	 * Maps AOS Agent thinking levels to provider/model-specific values.
