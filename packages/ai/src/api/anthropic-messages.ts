...
import { getRuntimeUserAgent } from "../utils/runtime-user-agent.ts";
 ...
 * Defaults to "short" and uses AOS_AGENT_CACHE_RETENTION for backward compatibility.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (getProviderEnvValue("AOS_AGENT_CACHE_RETENTION", env) === "long") {
		return "long";
	}
	return "short";
}
...
function mergeClientHeaders(
	...
	if (model.provider === "kimi-coding") {
		...
		merged["User-Agent"] = getRuntimeUserAgent("pi");
	}
	return merged;
}
		...
		try {
			...
			for await (const event of iterateAnthropicEvents(response, options?.signal)) {
				if (event.type === "message_start") {
					...
					output.model = event.message.model || model.id;
...
function createClient(
	...
	if (useServerSideFallbackBeta) betaFeatures.push(SERVER_SIDE_FALLBACK_BETA);
