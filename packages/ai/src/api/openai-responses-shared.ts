	...
	for (const msg of transformedMessages) {
		if (msg.role === "user") {
			...
			for (const block of msg.content) {
				if (block.type === "thinking") {
					...
					const fallbackMessageId =
						textBlockIndex === 0 ? `msg_aos_${msgIndex}` : `msg_aos_${msgIndex}_${textBlockIndex}`;
					textBlockIndex++;
			...
			if (deferredTools.length > 0 && options?.deferredToolsMode === "additional-tools") {
				...
				const searchCallId = `aos_tool_load_${shortHash(`${msg.toolCallId}:${names.join(",")}`)}`;
				messages.push({
...
export async function processResponsesStream<TApi extends Api>(
	...
	// multi-turn replay stateless. See upstream-issue
