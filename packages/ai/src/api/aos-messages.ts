/**
 * aos-messages API implementation.
 *
 * Streams AOS Agent's own message protocol directly to a backend: the request is a
 ...
 * models.json custom provider with `"api": "aos-messages"`.
 */
...
export interface AosMessagesOptions extends StreamOptions {
...
type AosMessagesUsage = AssistantMessage["usage"];
type AosMessagesStopReason = AssistantMessage["stopReason"];

/** Impact summary of a server-side message rewrite (e.g. a gateway policy). */
export type AosMessagesRewriteImpact = {
	policyId: string;
...
/** Serialized assistant-message event as sent by an aos-messages backend. */
export type AosMessagesEvent =
	| { type: "start" }
	...
	| {
			type: "done";
			reason: Extract<AosMessagesStopReason, "stop" | "length" | "toolUse">;
			usage: AosMessagesUsage;
			responseId?: string;
			rewrite?: AosMessagesRewriteImpact;
	  }
	| {
			type: "error";
			reason: Extract<AosMessagesStopReason, "aborted" | "error">;
			usage: AosMessagesUsage;
			errorMessage?: string;
			responseId?: string;
			rewrite?: AosMessagesRewriteImpact;
...
export class AosMessagesResponseError extends Error {
	code?: string;
	...
	constructor(message: string, code: string | undefined, diagnosticDetails: Record<string, unknown>) {
		super(message);
		this.name = "AosMessagesResponseError";
		this.code = code;
		this.diagnosticDetails = diagnosticDetails;
	}
}

function parseAosMessagesErrorBody(body: string): AosMessagesErrorBody | undefined {
	try {
		const parsed = JSON.parse(body) as AosMessagesErrorBody | null;
...
function formatAosMessagesResponseError(
	response: Response,
	body: string,
	errorBody: AosMessagesErrorBody | undefined,
): string {
...
function createAosMessagesResponseError(
	model: Model<"aos-messages">,
	url: URL,
	response: Response,
	body: string,
): AosMessagesResponseError {
	const errorBody = parseAosMessagesErrorBody(body);
	const code = typeof errorBody?.error?.code === "string" ? errorBody.error.code : undefined;
	return new AosMessagesResponseError(formatAosMessagesResponseError(response, body, errorBody), code, {
		version: 1,
...
function createEmptyUsage(): AosMessagesUsage {
	return {
		input: 0,
		output: 0,
		cacheRead: 0,
...
function appendRewriteDiagnostic(message: AssistantMessage, rewrite: AosMessagesRewriteImpact | undefined): void {
	if (!rewrite) {
		return;
	}
	appendAssistantMessageDiagnostic(message, {
		type: "aos_messages_rewrite",
		timestamp: Date.now(),
		details: { ...rewrite },
	});
}

function createEventConverter(model: Model<"aos-messages">) {
	...
	return (event: AosMessagesEvent): AssistantMessageEvent => {
		switch (event.type) {
			case "done":
...
async function* readAosMessagesEvents(stream: ReadableStream<Uint8Array>): AsyncGenerator<AosMessagesEvent> {
	...
	try {
		while (true) {
			...
			while (split !== -1) {
				const event = parseAosMessagesEvent(buffer.slice(0, split));
				if (event) {
					yield event;
				}
		...
		if (buffer.trim()) {
			const event = parseAosMessagesEvent(buffer);
			if (event) {
				yield event;
			}
		}
...
function parseAosMessagesEvent(raw: string): AosMessagesEvent | undefined {
	const data = raw
		.split("\n")
	...
	return data && data !== "[DONE]" ? (JSON.parse(data) as AosMessagesEvent) : undefined;
}

function createErrorEvent(model: Model<"aos-messages">, error: unknown, aborted: boolean): AssistantMessageEvent {
	...
	if (!aborted && error instanceof AosMessagesResponseError) {
		appendAssistantMessageDiagnostic(
			assistantMessage,
			createAssistantMessageDiagnostic("aos_messages_response_failure", error, error.diagnosticDetails),
		);
	}
...
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention | undefined {
	...
	// Backend defaults apply when unset; only the env opt-in is mapped.
	return getProviderEnvValue("AOS_AGENT_CACHE_RETENTION", env) === "long" ? "long" : undefined;
}

export const stream: StreamFunction<"aos-messages", AosMessagesOptions> = (
	model: Model<"aos-messages">,
	context: Context,
	options?: AosMessagesOptions,
	...
	void (async () => {
		try {
			...
			if (!response.ok) {
				...
				throw createAosMessagesResponseError(model, url, response, body);
			}
			if (!response.body) {
				throw new Error(`${model.provider} response has no body`);
			}

			for await (const aosEvent of readAosMessagesEvents(response.body)) {
				const event = convertEvent(aosEvent);
...
export const streamSimple: StreamFunction<"aos-messages", SimpleStreamOptions> = (
	model: Model<"aos-messages">,
	context: Context,
	options?: SimpleStreamOptions,
): AssistantMessageEventStream => {
	const extra = options as AosMessagesOptions | undefined;
