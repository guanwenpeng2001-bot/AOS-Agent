...
- `@aos-agent/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@aos-agent/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@aos-agent/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@aos-agent/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@aos-agent/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@aos-agent/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@aos-agent/ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `AOS_AGENT_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @aos-agent/ai login              # interactive provider selection
npx @aos-agent/ai login anthropic    # login to specific provider
npx @aos-agent/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@aos-agent/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@aos-agent/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@aos-agent/ai/compat';
```
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@aos-agent/ai`.
