...
import { type AosMessagesOptions, stream, streamSimple } from "../src/api/aos-messages.ts";
...
function createModel(baseUrl: string): Model<"aos-messages"> {
	return {
		id: "auto",
		name: "Radius Auto",
		api: "aos-messages",
...
describe("aos-messages", () => {
	...
	it("appends debug=1 and reports response headers via onResponse", async () => {
		...
		const options: AosMessagesOptions = {
			apiKey: "test-key",
			debug: true,
			onResponse: (response) => {
	...
	it("surfaces backend error responses with diagnostics", async () => {
		...
		expect(message.diagnostics?.[0]?.type).toBe("aos_messages_response_failure");
...
describe("aos-messages api registration", () => {
	it("is registered as a builtin api provider", async () => {
		const { getApiProvider } = await import("../src/compat.ts");
		expect(getApiProvider("aos-messages")).toBeDefined();
	});

	it("is a known api usable on models", () => {
		const api: Api = "aos-messages";
		expect(api).toBe("aos-messages");
	});
});
