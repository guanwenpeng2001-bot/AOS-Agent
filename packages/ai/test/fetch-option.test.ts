...
import { streamSimple as streamAosMessages } from "../src/api/aos-messages.ts";
...
describe("fetch stream option", () => {
	...
	it("uses fetch for Mistral, Codex SSE, and aos-messages HTTP requests", async () => {
		const { custom, fallback } = mockFetches();
		...
		await streamAosMessages(createModel("aos-messages"), context, {
			apiKey: "test-key",
			fetch: custom,
		}).result();
