...
function applyThinkingLevelMetadata(model: Model<any>): void {
	...
	if (model.provider === "openrouter" && model.id.startsWith("inception/mercury-2")) {
		...
		// AOS Agent's low/medium/high pass through verbatim; OpenRouter normalizes to Mercury's vocabulary.
		mergeThinkingLevelMap(model, { off: null });
	}
...
async function loadModelsDevData(): Promise<Model<any>[]> {
	try {
		...
		// provider reuses the international source and endpoint with a narrower catalog.
				// models.dev keys are "alibaba-token-plan[-cn]"; AOS Agent exposes them as
	...
	if (!generatorOptions.jsonOnly) {
		...
		try {
			...
			if (!generatorOptions.dataOnly) {
				...
				const previousAggregator = existsSync(aggregatorPath) ? readFileSync(aggregatorPath, "utf8") : undefined;
				restoreGeneratedCatalog = () => {
					...
					if (previousAggregator === undefined) {
						if (existsSync(aggregatorPath)) rmSync(aggregatorPath);
					} else {
