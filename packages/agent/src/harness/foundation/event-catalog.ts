...
export type WorkerLifecycleStatusV1 = "new" | "starting" | "ready" | "running" | "cancelling" | "completed" | "failed" | "cancelled" | "lost" | "reclaiming" | "reclaimed" | "reclaim_unknown";
export type WorkerLifecycleEventPayloadV1 = { schemaVersion: 1; workerId: string; providerId: string; sessionId: string; laneId: string; status: WorkerLifecycleStatusV1; revision: number; [key: string]: FoundationJsonValue };
export type WorkerOperationEventPayloadV1 = { schemaVersion: 1; workerId: string; providerId: string; sessionId: string; laneId: string; operationId: string; phase: string; revision: number; [key: string]: FoundationJsonValue };
...
export interface EventPayloadMapV1 {
	...
	"worker.lifecycle_transitioned": WorkerLifecycleEventPayloadV1; "worker.operation_recorded": WorkerOperationEventPayloadV1;
...
const correlationFor = (category: EventCategoryIdV1): readonly (keyof EventCorrelationRefV1)[] => {
	...
	if (category.startsWith("worker.")) return ["sessionId", "laneId", "workerId", "operationId", "runId", "bindingId", "bindingEpochId", "attemptId", "receiptId"];
...
const typedPayloadRules: Readonly<Record<string, EventPayloadRuleV1>> = {
	...
	"worker.lifecycle_transitioned": { required: ["schemaVersion", "workerId", "providerId", "sessionId", "laneId", "status", "revision"], allowed: ["schemaVersion", "workerId", "providerId", "sessionId", "laneId", "status", "revision", "runId", "bindingId", "bindingEpochId", "attemptId", "profileId", "createdAt", "readyAt", "endedAt", "lastHeartbeatAt", "activeOperationId", "operationId", "receiptId"], stringFields: ["workerId", "providerId", "sessionId", "laneId", "status"], integerFields: ["revision"], optionalStringFields: ["runId", "bindingId", "bindingEpochId", "attemptId", "profileId", "createdAt", "readyAt", "endedAt", "lastHeartbeatAt", "activeOperationId", "operationId", "receiptId"], stringValues: { status: ["new", "starting", "ready", "running", "cancelling", "completed", "failed", "cancelled", "lost", "reclaiming", "reclaimed", "reclaim_unknown"] } },
	"worker.operation_recorded": { required: ["schemaVersion", "workerId", "providerId", "sessionId", "laneId", "operationId", "phase", "revision"], allowed: ["schemaVersion", "workerId", "providerId", "sessionId", "laneId", "operationId", "phase", "revision", "sideEffectState", "receiptId", "recordedAt"], stringFields: ["workerId", "providerId", "sessionId", "laneId", "operationId", "phase"], integerFields: ["revision"], optionalStringFields: ["sideEffectState", "receiptId", "recordedAt"] },
...
function typedEventPayload(category: EventCategoryIdV1, value: unknown): value is FoundationJsonValue {
	...
	if (record.schemaVersion !== 1 || rule.stringFields.some((key) => typeof record[key] !== "string" || (record[key] as string).length === 0) || rule.optionalStringFields?.some((key) => record[key] !== undefined && (typeof record[key] !== "string" || (record[key] as string).length === 0)) || rule.integerFields.some((key) => typeof record[key] !== "number" || !Number.isInteger(record[key])) || Object.entries(rule.stringValues ?? {}).some(([key, allowed]) => typeof record[key] !== "string" || !allowed.includes(record[key] as string))) return false;
	return true;
}
...
function workerEventCorrelation(category: DurableEventCategoryV1, correlationValue: EventCorrelationRefV1, payloadValue: FoundationJsonValue): boolean {
	if (category !== "worker.lifecycle_transitioned" && category !== "worker.operation_recorded") return true;
	if (!plain(payloadValue)) return false;
	if (correlationValue.sessionId !== payloadValue.sessionId || correlationValue.laneId !== payloadValue.laneId || correlationValue.workerId !== payloadValue.workerId) return false;
	if (category === "worker.operation_recorded") return correlationValue.operationId === payloadValue.operationId;
	const operationId = payloadValue.operationId ?? payloadValue.activeOperationId;
	return operationId === undefined || correlationValue.operationId === operationId;
}
...
export function validateDurableEventV1(value: unknown): ResultValue<FoundationDurableEventV1, EventEnvelopeErrorV1> {
	...
	if (!stringValue(value.eventId) || !stringValue(value.streamId) || !stringValue(value.timestamp) || typeof value.sequence !== "number" || !Number.isInteger(value.sequence) || value.sequence < 0 || !correlation(value.correlation) || !jsonValue(value.payload) || !typedEventPayload(value.category, value.payload) || !workerEventCorrelation(value.category, value.correlation, value.payload)) return Result.err(new EventEnvelopeErrorV1("event_envelope_invalid_shape", "durable event fields are invalid"));
