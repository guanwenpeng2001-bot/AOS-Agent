...
import {
	...
	type RoleResolutionOverride,
	type RoleTombstone,
	validateRoleRegistryRecord,
} from "./role-registry.ts";
import { validateRoleRevision, type ModelProfile, type RoleRevision } from "./role.ts";
...
export const ROLE_REGISTRY_OBJECT_TYPE = "role_registry";
export const MODEL_PROFILE_OBJECT_TYPE = "model_profile";
...
function immutableRoleRevisionValid(value: RoleRevision): boolean {
	const { fingerprint, ...base } = value;
	return fingerprint.value === fingerprintFoundationValue(base).value;
}

function findRoleRevision(records: readonly RoleRegistryRecord[], roleRevisionId: string, revision: number): RoleRevision | undefined {
	for (const record of records) {
		for (const candidate of record.revisions) {
			const checked = validateRoleRevision(candidate);
...
function modelProfileFromPayload(payload: unknown, modelProfileId: string, revision: number): ModelProfile | undefined {
 ...
 */
export class DurableRoleRegistry {
	...
	private constructor(session: Session, options: DurableRoleRegistryOptions) {
		...
		this.ledger = new SessionLedger(session, { ownerId: options.ownerId });
	}

	static async create(session: Session, options: DurableRoleRegistryOptions = {}): Promise<DurableRoleRegistry> {
		const store = new DurableRoleRegistry(session, options);
		try {
	...
	async create(input: RoleRegistryCreateInput): Promise<ResultValue<RoleRegistryRecord, FoundationError>> {
		return this.mutate((registry) => registry.create(input));
	}

	async get(query: RoleRegistryGetQuery): Promise<ResultValue<RoleRegistryRecord, FoundationError>> {
		return this.read((registry) => registry.get(query));
	}

	async list(query: RoleRegistryListQuery = {}): Promise<ResultValue<readonly RoleRegistryRecord[], FoundationError>> {
		return this.read((registry) => registry.list(query));
	}

	async search(query: RoleRegistrySearchQuery): Promise<ResultValue<readonly RoleRegistryRecord[], FoundationError>> {
		return this.read((registry) => registry.search(query));
	}

	async edit(input: RoleRegistryEditInput): Promise<ResultValue<RoleRegistryRecord, FoundationError>> {
		return this.mutate((registry) => registry.edit(input));
	}

	async copy(input: RoleRegistryCopyInput): Promise<ResultValue<RoleRegistryRecord, FoundationError>> {
		return this.mutate((registry) => registry.copy(input));
	}

	async delete(input: RoleRegistryDeleteInput): Promise<ResultValue<RoleTombstone, FoundationError>> {
		return this.mutate((registry) => registry.delete(input));
	}

	async import(input: RoleRegistryImport): Promise<ResultValue<readonly RoleRegistryRecord[], FoundationError>> {
		return this.mutate((registry) => registry.import(input));
	}

	async export(query: RoleRegistryExportQuery = {}): Promise<ResultValue<RoleRegistryExport, FoundationError>> {
		return this.read((registry) => registry.export(query));
	}

	async resolve(input: RoleResolveInput): Promise<ResultValue<RoleResolutionPreview, FoundationError>> {
	...
	private async read<T>(operation: (registry: InMemoryRoleRegistry) => ResultValue<T, FoundationError>): Promise<ResultValue<T, FoundationError>> {
		await this.mutationTail;
		try {
			const registry = await this.loadRegistry();
	...
	private mutate<T>(operation: (registry: InMemoryRoleRegistry) => ResultValue<T, FoundationError>): Promise<ResultValue<T, FoundationError>> {
		const result = this.mutationTail.then(async () => {
			try {
				const beforeFacts = await this.loadRecordFacts();
				const before = beforeFacts.map((fact) => fact.payload);
				const registry = new InMemoryRoleRegistry({ now: this.now });
	...
	private async loadRegistry(): Promise<InMemoryRoleRegistry> {
		const registry = new InMemoryRoleRegistry({ now: this.now });
	...
	private async loadRecords(): Promise<readonly RoleRegistryRecord[]> {
	...
	private async validateDurableResolutionInput(input: RoleResolveInput): Promise<ResultValue<RoleResolveInput, FoundationError>> {
	...
	private async findDurableModelProfile(modelProfileId: string, revision: number): Promise<ModelProfile | undefined> {
		...
		const records = await this.ledger.find({ kind: "fact", objectType: MODEL_PROFILE_OBJECT_TYPE, order: "oldestFirst" });
		for (const record of records) {
			if (record.kind !== "fact") continue;
	...
	private async persistChanged(before: readonly StoredRoleRecordV1[], after: readonly RoleRegistryRecord[]): Promise<void> {
		for (const record of after) {
			...
			await this.ledger.appendFact(ROLE_REGISTRY_OBJECT_TYPE, roleObjectId(record.roleId, record.scope), record, {
...
export interface ModelProfileRecord {
	...
	revisions: readonly ModelProfile[];
}

export interface ModelProfilePutInput { profile: ModelProfile; }
export interface ModelProfileGetQuery { modelProfileId: string; revision?: number; }

/** Session-backed storage for secret-free, independently versioned ModelProfile revisions. */
export class DurableModelProfileStore {
	private readonly ledger: SessionLedger;
	private mutationTail: Promise<void> = Promise.resolve();
	...
	static async create(session: Session, options: DurableRoleRegistryOptions = {}): Promise<DurableModelProfileStore> {
		const store = new DurableModelProfileStore(session, options);
		try {
			await store.loadRecords();
			return store;
		} catch (error) {
	...
	async register(input: ModelProfilePutInput): Promise<ResultValue<ModelProfile, FoundationError>> {
		const result = this.mutationTail.then(async () => {
			try {
				const existingFact = await this.loadRecordFact(input.profile.modelProfileId);
				const existing = existingFact?.payload;
				const checked = validateSecretFreeModelProfile(input.profile);
				if (!checked.ok) return checked;
				const profile = checked.value;
				if (existing === undefined) {
					const record: ModelProfileRecord = { schemaVersion: 1, modelProfileId: profile.modelProfileId, currentRevision: profile, revisions: [profile] };
				...
				const next: ModelProfileRecord = { ...existing, currentRevision: profile, revisions: [...existing.revisions, profile] };
				await this.persist(next, existingFact?.revision ?? 0);
				return Result.ok(profile);
			} catch (error) {
				return resultError<ModelProfile>(error, "model_profile_persistence_failed");
			}
		});
		this.mutationTail = result.then(() => undefined, () => undefined);
		return result;
	...
	async get(query: ModelProfileGetQuery): Promise<ResultValue<ModelProfile, FoundationError>> {
		await this.mutationTail;
		try {
	...
	private async loadRecord(modelProfileId: string): Promise<ModelProfileRecord | undefined> {
		return (await this.loadRecordFact(modelProfileId))?.payload;
	}

	private async loadRecords(): Promise<ModelProfileRecord[]> {
		return (await this.loadRecordFacts()).map((fact) => fact.payload);
	}

	private async loadRecordFacts(): Promise<readonly { readonly payload: ModelProfileRecord; readonly revision: number }[]> {
		const facts = await this.ledger.find({ kind: "fact", objectType: MODEL_PROFILE_OBJECT_TYPE, order: "oldestFirst" });
		const records: { readonly payload: ModelProfileRecord; readonly revision: number }[] = [];
	...
	private async loadRecordFact(modelProfileId: string): Promise<{ readonly payload: ModelProfileRecord; readonly revision: number } | undefined> {
		const records = await this.loadRecordFacts();
		return records.find((record) => record.payload.modelProfileId === modelProfileId);
	}

	private async persist(record: ModelProfileRecord, expectedRevision: number): Promise<void> {
		await this.ledger.appendFact(MODEL_PROFILE_OBJECT_TYPE, record.modelProfileId, record, {
...
function validateModelProfileRecord(payload: unknown): ModelProfileRecord {
	...
	const current = validateSecretFreeModelProfile(candidate.currentRevision);
	...
	const revisions: ModelProfile[] = [];
	for (const revision of candidate.revisions) {
		const checked = validateSecretFreeModelProfile(revision);
