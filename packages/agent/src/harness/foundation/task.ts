...
export interface ModeSwitchIntentV1 { schemaVersion: 1; type: "role.switch"; modeSwitchId: string; taskId: string; attemptId: string; agentInstanceId: string; bindingId: string; activationReason: "mode_switch"; activatedByCommandId: string; createdAt: string; }
...
export function validateModeSwitchInput(input: ValidateModeSwitchInput): ResultValue<ModeSwitchIntentV1, FoundationError> {
	...
	if (input.currentEpoch.taskId !== input.intent.taskId || input.currentEpoch.bindingId !== input.intent.bindingId) return Result.err(new FoundationError("binding_epoch_mismatch", "Mode switch must retain the current task and binding", { details: { modeSwitchId: input.intent.modeSwitchId } }));
...
export const ModeSwitchIntentV1Schema = Type.Object({ schemaVersion: Type.Literal(1), type: Type.Literal("role.switch"), modeSwitchId: Type.String({ minLength: 1 }), taskId: Type.String({ minLength: 1 }), attemptId: Type.String({ minLength: 1 }), agentInstanceId: Type.String({ minLength: 1 }), bindingId: Type.String({ minLength: 1 }), activationReason: Type.Literal("mode_switch"), activatedByCommandId: Type.String({ minLength: 1 }), createdAt: Type.String({ minLength: 1 }) }, { additionalProperties: false });
