...
export function createBindingEpoch(input: CreateBindingEpochInput): ResultValue<BindingEpochV1, FoundationError> {
	...
	if (input.previous && input.previous.bindingId !== input.bindingId) return Result.err(new FoundationError("binding_epoch_mismatch", "Binding epochs cannot change binding identity inside an attempt", { details: { bindingEpochId: input.bindingEpochId } }));
