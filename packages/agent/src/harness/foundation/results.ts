...
/** Verified model identity observed by an exact external-connector handshake. */
export interface AttemptReceiptEffectiveModel { provider: string; model: string; bindingDigest: Fingerprint; observedAt: string; source: "claude_init_and_usage" | "codex_thread_start"; }
export interface AttemptReceipt { schemaVersion: 1; attemptReceiptId: string; taskId: string; dispatchId: string; attemptId: string; providerId: string; agentInstanceId?: string; bindingId: string; bindingEpochIds: readonly string[]; status: ResultStatus; workerReceiptRefs: readonly WorkerReceiptRef[]; artifacts: readonly ArtifactRef[]; usage?: AttemptReceiptUsage; effectiveModel?: AttemptReceiptEffectiveModel; error?: PublicExecutionError; provenance: ResultProvenance; sideEffectState: SideEffectState; }
...
export function validateAttemptReceipt(value: unknown, options: { agentProvider?: boolean; providerClass?: AttemptProviderClass } = {}): ResultValue<AttemptReceipt, FoundationError> {
	...
	if (receipt.effectiveModel !== undefined && (receipt.status !== "succeeded" || receipt.provenance.producerKind !== "external_connector")) return Result.err(new FoundationError("foundation_schema_invalid_shape", "Only a succeeded external connector receipt may carry an effective model", { details: { attemptReceiptId: receipt.attemptReceiptId } }));
