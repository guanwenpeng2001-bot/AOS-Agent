...
- Deep imports of `ask-store.ts`, `durable-store.ts`, `goal-store.ts`, `query.ts`, `workflow.ts`, and `workflow-store.ts` moved from `packages/agent/src/harness/foundation/` to `packages/coding-agent/src/orchestration/`.
