	...
	describe("bash", () => {
		...
		it("prepares command, cwd, and an explicit environment with the turn context", async () => {
			const env = new NodeExecutionEnv({
				cwd: createTempDir(),
				shellEnv: { AOS_AGENT_BASH_PREPARE_INHERITED: "inherited" },
			});
			...
			const tool = createBashTool<typeof context>({
				...
				prepare: async (execution, turnContext, signal) => {
					...
					execution.env = { AOS_AGENT_BASH_PREPARE_EXPLICIT: "explicit" };
					execution.inheritEnv = false;
					execution.command += `\nprintf '%s:%s:%s:%s' "$prefix" "\${AOS_AGENT_BASH_PREPARE_INHERITED-}" "$AOS_AGENT_BASH_PREPARE_EXPLICIT" "$PWD"`;
				},
			});
