...
describe("NodeExecutionEnv", () => {
	...
	it("expands home-relative paths and file URLs", async () => {
		const root = createTempDir();
		const env = new NodeExecutionEnv({ cwd: root });
		expect(getOrThrow(await env.absolutePath("~/aos-node-env-test"))).toBe(join(homedir(), "aos-node-env-test"));
	...
	it("executes commands in cwd with env overrides", async () => {
		const root = createTempDir();
		const env = new NodeExecutionEnv({ cwd: root });
		const printCwd = process.platform === "win32" ? '"$(pwd -W)"' : '"$PWD"';
		const result = getOrThrow(
			await env.exec(`printf '%s:%s' ${printCwd} "$NODE_ENV_TEST"`, {
				env: { NODE_ENV_TEST: "ok" },
			}),
		);
		const expectedCwd = await realpath(root);
		expect(result).toEqual({
			stdout: `${process.platform === "win32" ? expectedCwd.replaceAll("\\", "/") : expectedCwd}:ok`,
			stderr: "",
			exitCode: 0,
		});
	});
	...
	it.each([
		...
		["an empty override shadows the base value", { AOS_AGENT_SESSION_FILE: "" }, "x:"],
		[
	...
	it("can replace rather than inherit the default shell environment", async () => {
		const root = createTempDir();
		const inheritedKey = "AOS_AGENT_NODE_ENV_INHERITED_TEST";
		const configuredKey = "AOS_AGENT_NODE_ENV_CONFIGURED_TEST";
		const explicitKey = "AOS_AGENT_NODE_ENV_EXPLICIT_TEST";
