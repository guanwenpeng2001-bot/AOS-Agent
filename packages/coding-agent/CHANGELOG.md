...
- ModelBroker v1: trusted route/role settings, immutable model bindings and safe attempt/run metadata, RPC `get_model_routes` plus optional `modelRoute`/`modelRole`, SDK exports, and interactive `/model-routes`/`/model-route` controls.
