...
- MCP resources/prompts lifecycle and content normalizer: paginated `listResources` / `listResourceTemplates` / `listPrompts` with cursor forwarding, per-page item caps (an over-limit page is cut and its continuation cursor dropped), `readResource` and `getPrompt` with per-message normalization, secret-free resource/template/prompt views, advertised-capability gating (`resources` / `prompts`) that reports a redacted fixed message without degrading a server that never offered the capability, caller-signal cancellation with `AbortError` (never degrading), in-flight teardown on `forceClose`, stale-result rejection after a connection replacement, and redacted `unavailable` degradation on transport-level failures; a shared content normalizer (`normalizeContentBlocks` / `normalizeResourceContents`) that truncates text, drops oversized media, drops unrepresentable audio/transcript/link blocks, and caps total blocks under configurable `MCPContentLimits`, with the MCP tool adapter routed through the same normalizer; resource/prompt counts on the redacted connection status; and in-memory contract tests covering limits, pagination, cancellation, stale/degraded handling, and selection gates.
