...
import { DefaultPackageManager, type ProgressEvent, type ResolvedResource } from "../src/core/runtime/package-manager.ts";
...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.AOS_AGENT_OFFLINE;
		delete process.env.AOS_AGENT_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.AOS_AGENT_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .aos-agent skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .aos-agent", async () => {
			const extDir = join(tempDir, ".aos-agent", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".aos-agent"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".aos-agent", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".aos-agent", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".aos-agent", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".aos-agent", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".aos-agent", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .aos-agent/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-aos", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-aos"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-aos\ndescription: user AOS\n---\n");
		...
		it("should use the project .aos-agent dir as baseDir for project .aos-agent skills", async () => {
			const projectBaseDir = join(tempDir, ".aos-agent");
			const skillPath = join(projectBaseDir, "skills", "project-aos", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-aos"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-aos\ndescription: project AOS\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".aos-agent", "agent");
		...
		it("should dedupe user skill entries when ~/.aos-agent/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .aos-agent auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".aos-agent\n");

			const skillDir = join(tempDir, ".aos-agent", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.aos-agent\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".aos-agent", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the AOS-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "aos-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .aos-agent settings base", () => {
			...
			const rel = relative(join(tempDir, ".aos-agent"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled project package entries as deltas over global packages", async () => {
			const pkgDir = join(agentDir, "npm", "node_modules", "aos-tools");
			mkdirSync(join(pkgDir, "extensions"), { recursive: true });
			writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "aos-tools", version: "1.0.0" }));
			writeFileSync(join(pkgDir, "extensions", "foo.ts"), "export default function() {}");
			writeFileSync(join(pkgDir, "extensions", "bar.ts"), "export default function() {}");
			settingsManager.setPackages(["npm:aos-tools"]);
			settingsManager.setProjectPackages([
				{ source: "npm:aos-tools", autoload: false, extensions: ["-extensions/foo.ts"] },
			]);
			const runCommandSpy = vi
				.spyOn(packageManager as unknown as PackageManagerInternals, "runCommand")
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{ source: relative(join(tempDir, ".aos-agent"), pkgDir), autoload: false, extensions: ["+extensions/foo.ts"] },
			]);
...
export default function(api) { aagent.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
			);
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".aos-agent", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".aos-agent", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".aos-agent", "npm", "node_modules", "project-current");
