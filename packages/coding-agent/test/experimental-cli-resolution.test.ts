...
describe("experimental CLI command composition", () => {
	...
	test.each(["aos", "server", "client"] as const)("executes the parsed %s command", async (name) => {
		const context = {
			runAos: vi.fn(() => undefined),
			runServer: vi.fn(() => undefined),
			runClient: vi.fn(() => undefined),
		};
		const result = await experimentalCli.execute(name === "aos" ? [] : [name], context);

		expect(result).toMatchObject({ ok: true, command: { command: name } });
		expect(context.runAos).toHaveBeenCalledTimes(name === "aos" ? 1 : 0);
