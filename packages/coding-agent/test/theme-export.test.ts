...
describe("getThemeExportColors", () => {
	...
	afterEach(() => {
		...
		if (previousAgentDir === undefined) {
			delete process.env.AOS_AGENT_CODING_AGENT_DIR;
		} else {
			process.env.AOS_AGENT_CODING_AGENT_DIR = previousAgentDir;
		}
	});
	...
	it("resolves export variable references using the same syntax as colors", () => {
		...
		writeFileSync(
			join(process.env.AOS_AGENT_CODING_AGENT_DIR!, "themes", "custom-export-vars.json"),
	...
	it("resolves recursive vars and converts 256-color export values to hex", () => {
		...
		writeFileSync(
			join(process.env.AOS_AGENT_CODING_AGENT_DIR!, "themes", "custom-export-recursive.json"),
			JSON.stringify(customTheme, null, 2),
		);
