...
describe("AgentSession dynamic provider registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `aos-dynamic-provider-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const session = await createSession([
			(agent) => {
				agent.registerCommand("use-proxy", {
					...
					handler: async () => {
						agent.registerProvider("anthropic", { baseUrl: "http://localhost:8080/command" });
					},
				});
			},
		]);
		...
		const session = await createSession([
			(agent) => {
				agent.registerCommand("use-native", {
					...
					handler: async () => {
						agent.registerProvider(nativeAnthropicProvider("http://localhost:8080/native-command"));
					},
				});
			},
		]);
