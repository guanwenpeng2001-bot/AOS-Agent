...
- Use the fake provider from `packages/ai/src/providers/fake.ts`
- Do not use real provider APIs, real API keys, network calls, or paid tokens
- Keep these tests CI-safe and deterministic
- Do not use or extend the legacy `test/session/test-harness.ts` path unless a missing capability forces it
