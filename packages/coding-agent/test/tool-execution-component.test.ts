...
import { Text, type TUI } from "@aos-agent/tui";
...
describe("ToolExecutionComponent parity", () => {
	...
	test("collapses fallback results until expanded", () => {
		...
		const output = Array.from({ length: 15 }, (_, index) => `row-${index + 1}`).join("\n");
		component.updateResult({ content: [{ type: "text", text: output }], details: {}, isError: false }, false);
		const collapsed = stripAnsi(component.render(120).join("\n"));
		expect(collapsed).toContain("row-10");
	...
	for (const scenario of [
		...
		{
			title: "AGENTS.md",
			path: join(process.cwd(), ".aos-agent", "AGENTS.md"),
			content: "Hidden resource instructions",
			compact: "read resource .aos-agent/AGENTS.md",
			hidden: "Hidden resource instructions",
			absent: undefined,
		},
		{
			title: "AGENTS.override.md",
			path: join(process.cwd(), ".aos-agent", "AGENTS.override.md"),
			content: "Hidden override instructions",
			compact: "read resource .aos-agent/AGENTS.override.md",
	...
	for (const scenario of [
		...
		{ title: "AOS Agent documentation", path: getReadmePath(), compact: "read docs README.md:120-329" },
	] as const) {
