...
describe("coding-agent Harness construction", () => {
	...
	test("sets the optional session file in the default bash tool environment", async () => {
		...
		try {
			...
			expect(env.executionOverrides).toEqual({
				AOS_AGENT_SESSION_ID: "session-file-harness",
				AOS_AGENT_SESSION_FILE: "/sessions/current.jsonl",
				AOS_AGENT_PROVIDER: "google",
				AOS_AGENT_MODEL: "gemini-2.5-flash",
				AOS_AGENT_REASONING_LEVEL: "high",
			});
	...
	test("keeps bash AOS model variables synchronized with Harness state", async () => {
		...
		try {
			...
			expect(env.executionOverrides).toEqual({
				AOS_AGENT_SESSION_ID: "dynamic-bash-session",
				AOS_AGENT_SESSION_FILE: "",
				AOS_AGENT_PROVIDER: "anthropic",
				AOS_AGENT_MODEL: "claude-sonnet-4-5",
				AOS_AGENT_REASONING_LEVEL: "low",
			});
			expect(Object.hasOwn(env.executionOverrides ?? {}, "AOS_AGENT_SESSION_FILE")).toBe(true);
			expect(env.executionOverrides?.AOS_AGENT_SESSION_FILE).toBe("");
			expect(result.content).toEqual([
				{
					type: "text",
