...
import { hasTrustRequiringProjectResources, ProjectTrustStore } from "../src/core/policy/trust-manager.ts";
...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".aos-agent", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".aos-agent", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".aos-agent", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".aos-agent"), { recursive: true });
			writeFileSync(join(cwd, ".aos-agent", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".aos-agent"), { recursive: true, force: true });
