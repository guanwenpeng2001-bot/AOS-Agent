...
beforeEach(() => {
	allowNetwork();
	tempDir = join(tmpdir(), `aos-test-radius-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("Radius provider", () => {
	it("restores the legacy credential catalog without network access", async () => {
		...
		expect(model).toMatchObject({ api: "aos-messages", baseUrl: "https://radius.example.com/v1" });
	...
	it("fetches and stores the catalog for configured Radius auth", async () => {
		const gatewayBase = "https://radius.example.com";
		vi.spyOn(globalThis, "fetch").mockImplementation(
			async () =>
				new Response(JSON.stringify(radiusConfig(`${gatewayBase}/v1`)), {
					status: 200,
					headers: { "content-type": "application/json" },
				}),
		);
		const modelsStore = new InMemoryModelsStore();
		const modelsPath = join(tempDir, "models-radius.json");
		writeFileSync(
			modelsPath,
			JSON.stringify({
				providers: {
					[RADIUS_PROVIDER_ID]: { name: "Radius", baseUrl: gatewayBase, oauth: "radius" },
				},
			}),
		);
		...
		const radiusRequest = vi
			.mocked(fetch)
			.mock.calls.find(([url]) => String(url) === `${gatewayBase}/v1/config`);
