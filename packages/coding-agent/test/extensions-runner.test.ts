...
describe("ExtensionRunner", () => {
	...
	agent.on("project_trust", () => ({ trusted: "undecided", remember: true }));
}`,
			);
			fs.writeFileSync(
				decidedPath,
				`export default function(agent) {
	agent.on("project_trust", () => ({ trusted: "no", remember: true }));
}`,
			);
	...
	describe("shortcut conflicts", () => {
		...
		it("warns but allows when extension uses non-reserved built-in shortcut", async () => {
			...
			const extCode = `
				export default function(agent) {
					agent.registerShortcut("${pasteImageKey}", {
		...
		it("warns when two extensions register same shortcut", async () => {
			...
			const extCode1 = `
				export default function(agent) {
					agent.registerShortcut("ctrl+shift+x", {
	...
	describe("message and entry renderers", () => {
		it("gets Markdown transformers in extension load order", async () => {
			const extCode = `
				export default function(agent) {
					agent.registerMarkdownTransformer((markdown) => markdown);
				}
			`;
		...
		it("gets message renderer by type", async () => {
			const extCode = `
				export default function(agent) {
					agent.registerMessageRenderer("my-type", (message, options, theme) => null);
				}
			`;
		...
		it("gets entry renderer by type", async () => {
			const extCode = `
				export default function(agent) {
					agent.registerEntryRenderer("my-entry", (entry, options, theme) => null);
				}
			`;
	...
	describe("flags", () => {
		...
		it("keeps first flag when two extensions register the same name", async () => {
			const first = `
				export default function(agent) {
					agent.registerFlag("shared-flag", {
						description: "first",
						type: "boolean",
						default: true,
					});
		...
		it("rejects default values that do not match the flag type", async () => {
			const extCode = `
				export default function(pi) {
					pi.registerFlag("safe-mode", { type: "boolean", default: "false" });
				}
			`;
	...
	describe("hasHandlers", () => {
		it("returns true when handlers exist for event type", async () => {
			const extCode = `
				export default function(agent) {
					agent.on("tool_call", async () => undefined);
				}
			`;
	...
	describe("before_provider_headers", () => {
		...
		it("isolates a throwing handler and still applies the others", async () => {
			const throwing = `
				export default function(agent) {
					agent.on("before_provider_headers", () => {
