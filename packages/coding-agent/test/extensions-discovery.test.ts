...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aos-ext-test-"));
	...
	it("loads the coding-agent entrypoint without rewriting @aos-agent/ai provider subpaths", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "coding-agent-import.ts"),
			`
				import { getAgentDir } from "aos-agent";
				void getAgentDir;
				export default function(agent) {
					agent.registerCommand("test", { handler: async () => {} });
				}
	...
	it("keeps the type-only legacy AI OAuth compatibility barrel resolvable", async () => {
		fs.writeFileSync(
			path.join(extensionsDir, "oauth-import.ts"),
			`
				import * as oauth from "@aos-agent/ai/oauth";
				void oauth;
				export default function(agent) {
					agent.registerCommand("test", { handler: async () => {} });
				}
	...
	it("registers message and entry renderers", async () => {
		const extCode = `
			export default function(agent) {
				agent.registerMarkdownTransformer((markdown) => {
					return markdown;
				});
