...
describe("AgentSession dynamic tool registration", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `aos-dynamic-tool-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("exposes session state before custom bash spawn hooks and supports opting out", async () => {
		...
		expect(sessionEnv).toMatchObject({
			AOS_AGENT_SESSION_ID: session.sessionId,
			AOS_AGENT_SESSION_FILE: session.sessionFile,
			AOS_AGENT_PROVIDER: model.provider,
			AOS_AGENT_MODEL: model.id,
			AOS_AGENT_REASONING_LEVEL: session.thinkingLevel,
		});
		...
		expect(optedOutEnv).not.toHaveProperty("AOS_AGENT_SESSION_ID");
		expect(optedOutEnv).not.toHaveProperty("AOS_AGENT_SESSION_FILE");
		expect(optedOutEnv).not.toHaveProperty("AOS_AGENT_PROVIDER");
		expect(optedOutEnv).not.toHaveProperty("AOS_AGENT_MODEL");
		expect(optedOutEnv).not.toHaveProperty("AOS_AGENT_REASONING_LEVEL");
