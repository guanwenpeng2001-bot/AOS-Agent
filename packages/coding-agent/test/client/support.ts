import { type ByteTransport, type ByteTransportHandlers, AosClient } from "@aos-agent/client";
import {
	type ClientMessage,
...
export async function connectClient(server: MemoryServer): Promise<AosClient> {
	const client = new AosClient({ transportFactory: (handlers) => server.connect(handlers) });
