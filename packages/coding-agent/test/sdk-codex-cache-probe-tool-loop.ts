...
function parseArgs(argv: string[]): Args {
	...
	let sessionPath = resolve(join(tmpdir(), `aos-sdk-codex-cache-probe-tool-loop-${Date.now()}.jsonl`));
...
function buildPrompt(turn: number): string {
	...
	const lines: string[] = [];
	lines.push("This is an automated prompt-cache tool-loop test executed through the AOS Agent SDK.");
