...
import type { ResolvedPaths } from "../src/core/runtime/package-manager.ts";
...
describe("package commands", () => {
	...
	beforeEach(() => {
		allowNetwork();
		tempDir = join(tmpdir(), `aos-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("skips untrusted project package settings", async () => {
		mkdirSync(join(projectDir, ".aos-agent"), { recursive: true });
		writeFileSync(join(projectDir, ".aos-agent", "settings.json"), JSON.stringify({ packages: ["npm:@project/pkg"] }));
	...
	it("blocks local package changes when project is untrusted", async () => {
		mkdirSync(join(projectDir, ".aos-agent"), { recursive: true });
		writeFileSync(join(projectDir, ".aos-agent", "settings.json"), "{}");
	...
	it("allows local package install to initialize fresh project settings", async () => {
		...
		expect(settings.packages?.length).toBe(1);
		const stored = settings.packages?.[0] ?? "";
		expect(realpathSync(join(projectDir, ".aos-agent", stored))).toBe(realpathSync(packageDir));
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("aos install <source> [-l]");
	...
	it("cycles project package overrides in config local mode", async () => {
		...
		storage.withLock("global", () => JSON.stringify({ packages: ["npm:aos-tools"] }));
		const settingsManager = SettingsManager.fromStorage(storage, { projectTrusted: true });
		const resolvedPaths = extensionPaths(join(tempDir, "pkg"), "npm:aos-tools", "user", ["bar.ts"]);
		...
		expect(settingsManager.getProjectSettings().packages).toEqual([
			{ source: "npm:aos-tools", autoload: false, extensions: [join("-extensions", "bar.ts")] },
		]);
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "aos --help" or "aos install <source> [-l] [--approve|--no-approve]".');
			expect(process.exitCode).toBe(1);
		} finally {
			errorSpy.mockRestore();
		}
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: aos install <source> [-l]");
	...
	it("allows explicit self-update checks when automatic version checks are disabled", async () => {
		const previousSkipVersionCheck = process.env.AOS_AGENT_SKIP_VERSION_CHECK;
		...
		try {
			...
			if (previousSkipVersionCheck === undefined) {
				...
				process.env.AOS_AGENT_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			}
		}
	});
	...
	it("retries a transient self-update version check", async () => {
		...
		try {
			...
			if (previousSkipVersionCheck === undefined) delete process.env.AOS_AGENT_SKIP_VERSION_CHECK;
			else process.env.AOS_AGENT_SKIP_VERSION_CHECK = previousSkipVersionCheck;
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
	});
	...
	it("uses the update check version for forced self updates even when current", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@aos-agent", "aos-agent");
...
`,
		...
		process.env.AOS_AGENT_PACKAGE_DIR = selfPackageDir;
...
`,
		...
		const activePackageName = PACKAGE_NAME === "@new-scope/aos-agent" ? "@newer-scope/aos-agent" : "@new-scope/aos-agent";
		vi.stubGlobal(
			"fetch",
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		const selfPackageDir = join(globalRoot, "node_modules", "@aos-agent", "aos-agent");
...
`,
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:aos-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "aos-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:aos-formatter?");
			expect(stdout).not.toContain("Updated aos-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:aos-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
