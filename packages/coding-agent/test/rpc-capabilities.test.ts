...
async function startPublicBoundaryHarness(): Promise<{
	...
	const session = {
		...
		inspectCapabilityCatalog: () => ({
			...
			descriptors: [
				{
					id: `skill:${OPAQUE_SOURCE_ID}:audit`,
					revision: OPAQUE_REVISION_ID,
					kind: "skill",
					name: "audit",
					source: { source: OPAQUE_SOURCE_ID, scope: "user", origin: "package" },
...
describe("RPC capability public boundary", () => {
	...
	it("does not leak raw capability sources through actual RPC JSONL read commands", async () => {
		...
		try {
			...
			expect((capabilities.catalog as { descriptors?: Array<{ source?: { source?: string } }> }).descriptors?.[0]?.source?.source).toBe(
				OPAQUE_SOURCE_ID,
			);
