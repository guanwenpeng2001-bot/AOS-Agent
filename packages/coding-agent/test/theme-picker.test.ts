...
describe("theme picker", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "aos-theme-picker-"));
		const agentDir = join(tempRoot, "agent");
		vi.stubEnv("AOS_AGENT_CODING_AGENT_DIR", agentDir);
	...
	it("uses custom theme content names instead of file names", () => {
		...
		const themePath = join(process.env.AOS_AGENT_CODING_AGENT_DIR!, "themes", "foo.json");
