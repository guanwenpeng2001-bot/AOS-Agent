...
// ============================================================================
// OAuth API key resolution from ~/.aos-agent/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".aos-agent", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.aos-agent/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.aos-agent/agent/auth.json
 */
...
/** Path to the real AOS Agent config directory */
export const AOS_AGENT_DIR = join(homedir(), ".aos-agent", "agent");

/**
 * Get an AuthStorage instance backed by ~/.aos-agent/agent/auth.json
...
export async function createTestSession(options: TestSessionOptions = {}): Promise<TestSessionContext> {
	const tempDir = join(tmpdir(), `aos-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
