...
import { Markdown, type MarkdownTheme } from "@aos-agent/tui";
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|aos] [--self|--extensions|--models|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Update AOS Agent, installed packages, or model catalogs.

Options:
  --self                  Update AOS Agent only (default when no target is given)
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update AOS Agent and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall AOS Agent even if the current version is latest

Short forms:
  ${APP_NAME} update                Update AOS Agent only
  ${APP_NAME} update --all          Update AOS Agent and all extensions
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update aos             Update AOS Agent only (self works as an alias)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (modelsFlag) {
			...
			const sourceIsSelf = source === "self" || source === APP_NAME || source === "aos";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of aos executable: ${entrypoint}`);
	}
}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestAosRelease>>;
	try {
		latestRelease = await getLatestAosRelease(VERSION, { retry: true });
	} catch (error: unknown) {
