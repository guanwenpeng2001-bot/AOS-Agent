...
const EXTENSION_LOAD_FAILURE_HINT = 'Hint: Start without extensions using "aos -ne".';
...
export async function main(args: string[], options?: MainOptions) {
	...
	if (await handlePackageCommand(args, { extensionFactories })) {
		...
		if (process.platform === "win32" && exitCode === 0 && args[0] === "update") {
			...
			// runs during teardown; let successful `aos update` drain naturally instead.
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.AOS_AGENT_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: AOS_AGENT_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
	...
	if (appMode === "rpc") {
		printTimings();
		await runRpcMode(runtime, parsed.rpcListen === undefined ? undefined : { listen: parsed.rpcListen });
