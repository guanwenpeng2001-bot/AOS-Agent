...
import * as _bundledPiAgentCore from "@aos-agent/agent-core";
import type { Provider } from "@aos-agent/ai";
import * as _bundledPiAiCompat from "@aos-agent/ai/compat";
import * as _bundledPiAiOauth from "@aos-agent/ai/oauth";
import * as _bundledPiAiProviders from "@aos-agent/ai/providers/all";
import type { KeyId } from "@aos-agent/tui";
import * as _bundledPiTui from "@aos-agent/tui";
...
// avoiding a circular dependency. Extensions can import from aos-agent.
...
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	"@aos-agent/agent-core": _bundledPiAgentCore,
	"@aos-agent/tui": _bundledPiTui,
	"@aos-agent/ai": _bundledPiAiCompat,
	"@aos-agent/ai/compat": _bundledPiAiCompat,
	"@aos-agent/ai/oauth": _bundledPiAiOauth,
	"@aos-agent/ai/providers/all": _bundledPiAiProviders,
	"aos-agent": _bundledAosCodingAgent,
};
...
function getAliases(): Record<string, string> {
	...
	const piAgentCoreEntry = resolveWorkspaceOrImport("agent/dist/index.js", "@aos-agent/agent-core");
	const piTuiEntry = resolveWorkspaceOrImport("tui/dist/index.js", "@aos-agent/tui");
	// Extensions resolve the ai package root to the compat entrypoint (a strict
	// superset of the core entrypoint): existing extensions using the old
	// global API keep working at runtime until compat is removed.
	const piAiCompatEntry = resolveWorkspaceOrImport("ai/dist/compat.js", "@aos-agent/ai/compat");
	const piAiOauthEntry = resolveWorkspaceOrImport("ai/dist/oauth.js", "@aos-agent/ai/oauth");
	const piAiProvidersEntry = resolveWorkspaceOrImport(
		"ai/dist/providers/all.js",
		"@aos-agent/ai/providers/all",
	);

	_aliases = {
		"aos-agent": piCodingAgentEntry,
		"@aos-agent/agent-core": piAgentCoreEntry,
		"@aos-agent/tui": piTuiEntry,
		"@aos-agent/ai/providers/all": piAiProvidersEntry,
		"@aos-agent/ai/compat": piAiCompatEntry,
		"@aos-agent/ai/oauth": piAiOauthEntry,
		"@aos-agent/ai": piAiCompatEntry,
 ...
 *
 * Checks for:
 * 1. package.json with "aosAgent.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
	// Check for package.json with "aosAgent" field first
	const packageJsonPath = path.join(dir, "package.json");
	if (fs.existsSync(packageJsonPath)) {
		const manifest = readAosAgentManifest(packageJsonPath);
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "aosAgent" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
	...
	for (const p of configuredPaths) {
		...
		if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
			// Check for package.json with aosAgent manifest or index.ts
