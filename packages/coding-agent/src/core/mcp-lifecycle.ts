...
/**
 * Builds the stdio SDK transport. The child process environment is the SDK's
 * curated safe default set merged with the explicitly allowlisted environment
 * variable names; the parent process environment is never inherited wholesale.
 */
export function createMCPStdioTransport(
	...
	const envAllowlist = config.env ?? [];
	if (envAllowlist.length > 0) {
		const resolvedEnv: Record<string, string> = {};
		for (const [name, value] of Object.entries(getDefaultEnvironment())) {
