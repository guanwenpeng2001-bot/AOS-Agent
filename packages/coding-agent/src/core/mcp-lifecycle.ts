...
export class MCPServerLifecycle {
	...
	async readResource(uri: string, signal?: AbortSignal): Promise<MCPReadResourceResult> {
		const client = this.requireReady();
		const safeUri = validateMCPResourceUri(uri, this.config.id);
		if (client.getServerCapabilities()?.resources === undefined) {
			throw new MCPContentError("mcp_resource_unavailable", this.config.id);
		}
		return this.runCatalogOp(async (controller) => {
	...
	async getPrompt(
		...
		const client = this.requireReady();
		const safeName = validateMCPPromptName(name, this.config.id);
	 ...
	 * Caller cancellation rejects with AbortError without degrading; any other
	 * failure (transport or content) marks the server degraded and throws the
	 * fixed error or capability_mcp_unavailable.
	 */
