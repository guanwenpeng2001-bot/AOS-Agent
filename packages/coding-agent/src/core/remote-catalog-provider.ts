import type { Api, Model, ModelsStoreEntry, Provider } from "@aos-agent/ai";
...
/** Add a persisted remote catalog overlay to a static built-in provider. Requires an explicit catalog base URL. */
export function withRemoteCatalog(
	provider: Provider,
	catalogBaseUrl: string,
	localGeneratedAt?: number,
): Provider {
	const baseUrl = catalogBaseUrl.trim();
	if (!baseUrl) {
		return provider;
	}
