...
import { RuntimeCredentials } from "./credentials.ts";
...
/** Configured @aos-agent/ai Models collection used by coding-agent and SDK consumers. */
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		...
		const catalogBaseUrl = options.catalogBaseUrl?.trim();
		const providers = builtinProviderCatalog
			.builtinProviders()
			.map((provider) =>
				provider.id === "radius" || !catalogBaseUrl
					? provider
		...
		const runtime = new ModelRuntime(
			...
			modelsStore,
			providers,
			process.env.AOS_AGENT_OFFLINE === undefined,
		);
	...
	private async runModelRefresh(options: ModelsRefreshOptions): Promise<ModelsRefreshResult> {
		...
		// Older compatibility builds returned void and accepted a provider ID before ModelsStore.
