import type { Api, Model, ProviderHeaders } from "@aos-agent/ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://aos-agent",
			"X-OpenRouter-Title": "AOS Agent",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "AOS Agent",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "aos-agent",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "aos-agent" };
}
