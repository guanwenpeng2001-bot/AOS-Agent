...
import { type AosAgentManifest, readAosAgentManifest } from "./aos-agent-manifest.ts";
...
function isOfflineModeEnabled(): boolean {
	const value = process.env.AOS_AGENT_OFFLINE;
...
function expandPackageGlob(pattern: string, root: string): string[] {
	return globSync(pattern, { cwd: root })
		...
		.filter((path) => relative(root, path).split(sep).every((segment) => segment === ".." || !segment.startsWith(".")))
...
type SkillDiscoveryMode = "aos" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "aos" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "aos");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		const packageManagerName = this.getPackageManagerName();
			// Extension packages run inside AOS Agent and resolve extension APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// Stale auto-installed workspace peers can otherwise block updates.
	...
	private getGitUpdateMarkerPath(targetDir: string): string {
		return join(dirname(targetDir), `.${basename(targetDir)}.aos-agent-update-incomplete`);
	}
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "aos-agent-extensions", private: true };
	...
	private collectPackageResources(
		...
		const manifest = readAosAgentManifest(join(packageRoot, "package.json"));
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof AosAgentManifest];
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			// Project extensions from .aos-agent/
			addResources(
				"extensions",
			...
			// Project skills from .aos-agent/
			addResources(
				"skills",
				collectAutoSkillEntries(projectDirs.skills, "aos"),
				projectMetadata,
		...
		// User extensions from ~/.aos-agent/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.aos-agent/agent/
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "aos"),
			userMetadata,
