...
import { Container, Text, truncateToWidth } from "@aos-agent/tui";
...
export const bashToolSystemPromptContribution = {
	...
	guidelines: ["You can inspect AOS_AGENT_* environment variables for current model and session details."],
} as const;
...
/**
 * Create bash operations using aos's built-in local shell execution backend.
 *
 * This is useful for extensions that intercept user_bash and still want aos's
...
function resolveSpawnContext(
	...
	delete env.AOS_AGENT_SESSION_ID;
	delete env.AOS_AGENT_SESSION_FILE;
	delete env.AOS_AGENT_PROVIDER;
	delete env.AOS_AGENT_MODEL;
	...
	if (exposeSessionEnvironment && ctx) {
		...
		if (model) {
			...
			env.AOS_AGENT_MODEL = model.id;
		}
		if (ctx.thinkingLevel) env.AOS_AGENT_REASONING_LEVEL = ctx.thinkingLevel;
	}
...
export interface BashToolOptions {
	...
	shellPath?: string;
	/** Expose current AOS Agent session metadata as AOS_AGENT_* environment variables. Default: true */
	exposeSessionEnvironment?: boolean;
...
export function createBashToolDefinition(
	...
	return {
		...
		async execute(
			...
			const output = new OutputAccumulator({ tempFilePrefix: "aos-bash" });
