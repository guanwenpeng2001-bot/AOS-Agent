...
export type SessionCreationOptions =
	...
	| { readonly mode: "new"; readonly directory?: string; readonly id?: string }
...
export function createSessionManagerForOptions(options: {
	...
	const sessionManager = openedSessionManager ?? (options.session?.mode === "memory"
		? SessionManager.inMemory(cwd, options.session.id === undefined ? undefined : { id: options.session.id })
		: options.session?.mode === "continue"
			...
			: SessionManager.create(
				...
				options.session?.mode === "new" && options.session.id !== undefined
