...
export interface AosCommand {
	readonly command: "aos";
	readonly auth?: AuthInput;
	readonly options: Args;
	readonly listen?: readonly TransportAddress[];
}

export interface AosCommandContext {
	runAos(command: AosCommand): void | Promise<void>;
}

const listenOption = transportOption("--listen");

export const aosCommand = new Command<AosCommand, AosCommandContext>("aos")
	.option(listenOption)
	...
	.action((command, context) => context.runAos(command));
