...
const OFFICIAL_PACKAGE_NAME = "aos-agent";
const OFFICIAL_APP_NAME = "aos";
const OFFICIAL_CONFIG_DIR_NAME = ".aos-agent";
 ...
 * - this is the official AOS Agent distribution
 * - experimental features are enabled (AOS_AGENT_EXPERIMENTAL=1)
...
export function shouldRunFirstTimeSetup(settingsPath: string = getSettingsPath()): boolean {
	...
	if (process.env[ENV_CODING_AGENT_DIR]) {
		return false;
	}
