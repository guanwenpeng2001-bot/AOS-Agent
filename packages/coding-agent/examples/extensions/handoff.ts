...
import { type Message, uuidv7 } from "@aos-agent/ai";
import type { ExtensionAPI, SessionEntry } from "aos-agent";
import { BorderedLoader, convertToLlm, serializeConversation } from "aos-agent";
...
export default function (agent: ExtensionAPI) {
	agent.registerCommand("handoff", {
		...
		handler: async (args, ctx) => {
			...
			const currentSessionFile = ctx.session.getSessionFile();
