...
export default function (agent: ExtensionAPI) {
	...
	ctx.ui.setTitle(event.reason === "new" ? "AOS Agent RPC Demo (new session)" : "AOS Agent RPC Demo");
	...
	agent.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	agent.on("turn_end", async (_event, ctx) => {
	...
	agent.registerCommand("rpc-input", {
	...
	agent.registerCommand("rpc-editor", {
	...
	agent.registerCommand("rpc-prefill", {
