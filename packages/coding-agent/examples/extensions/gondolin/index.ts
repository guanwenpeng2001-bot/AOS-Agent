 ...
 *
 * Runs AOS Agent's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   aos -e /path/to/aos-agent/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (agent: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `aos ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
	...
	agent.on("session_shutdown", async (_event, ctx) => {
		const activeVm = vm;
		vm = undefined;
		vmStarting = undefined;
		if (!activeVm) return;
	...
	agent.registerCommand("gondolin", {
	...
	agent.on("user_bash", async (_event, ctx) => {
		const activeVm = await ensureVm(ctx);
