 ...
 *
 * Usage:
 *   aos -e ./ssh.ts --ssh user@host
 *   aos -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (agent: ExtensionAPI) {
	agent.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	agent.on("session_start", async (_event, ctx) => {
		...
		const arg = agent.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
