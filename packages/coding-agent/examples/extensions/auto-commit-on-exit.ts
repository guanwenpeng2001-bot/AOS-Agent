...
export default function (agent: ExtensionAPI) {
	agent.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await agent.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[aos] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await agent.exec("git", ["add", "-A"]);
		const { code: commitCode } = await agent.exec("git", ["commit", "-m", commitMessage]);
