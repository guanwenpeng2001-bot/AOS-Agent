 ...
 *
 * Usage: aos --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "aos-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@aos-agent/tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@aos-agent/tui";
...
export default function (agent: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates for a game-like workload.
	agent.registerCommand("overlay-animation", {
	...
	agent.registerCommand("overlay-anchors", {
	...
	agent.registerCommand("overlay-margins", {
	...
	agent.registerCommand("overlay-stack", {
	...
	agent.registerCommand("overlay-overflow", {
	...
	agent.registerCommand("overlay-edge", {
	...
	agent.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	agent.registerCommand("overlay-maxheight", {
	...
	agent.registerCommand("overlay-sidepanel", {
	...
	agent.registerCommand("overlay-toggle", {
	...
	agent.registerCommand("overlay-passive", {
	...
	agent.registerCommand("overlay-focus", {
	...
	agent.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/path/to/aos-agent/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /path/to/aos-agent\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (a game-like renderer uses the same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
