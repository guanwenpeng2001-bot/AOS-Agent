 ...
 *
 * Start AOS Agent with this extension:
 *   aos -e ./examples/extensions/git-merge-and-resolve.ts
 */
...
async function findConflicts(agent: ExtensionAPI, cwd: string): Promise<ConflictBlock[]> {
	const { stdout, code } = await agent.exec("git", ["diff", "--name-only", "--diff-filter=U"]);
...
export default function (agent: ExtensionAPI) {
	agent.on("agent_end", async (_event, ctx) => {
		const { code: revParseCode } = await agent.exec("git", ["rev-parse", "--git-dir"]);
		if (revParseCode !== 0) return;

		let ref = "MERGE_HEAD";

		// If not already in a merge, attempt one
		const { code: mergeHeadCode } = await agent.exec("git", ["rev-parse", "MERGE_HEAD"]);
		if (mergeHeadCode !== 0) {
			// Only attempt a new merge if the working tree is clean
			const { stdout: status } = await agent.exec("git", ["status", "--porcelain"]);
			if (status.trim()) return;

			const { stdout: upstream, code: upstreamCode } = await agent.exec("git", [
				"rev-parse",
				"--abbrev-ref",
				"--symbolic-full-name",
				"@{u}",
			...
			const { code: fetchCode, stderr: fetchErr } = await agent.exec("git", ["fetch", remote]);
			if (fetchCode !== 0) {
				ctx.ui.notify(`git-merge-and-resolve: fetch failed: ${fetchErr.trim()}`, "warning");
				return;
			}

			const { code: mergeCode } = await agent.exec("git", ["merge", "--no-ff", ref]);
