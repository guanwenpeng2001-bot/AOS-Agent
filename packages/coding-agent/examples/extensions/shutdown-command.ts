 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit AOS Agent cleanly.
 */
...
export default function (agent: ExtensionAPI) {
	// Register a /quit command that cleanly exits AOS Agent
	agent.registerCommand("quit", {
		description: "Exit AOS Agent cleanly",
	...
	agent.registerTool({
		...
		description: "Complete a task and exit AOS Agent",
	...
	agent.registerTool({
		...
		description: "Deploy the application and exit AOS Agent",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await agent.exec("npm", ["run", "deploy", params.environment], { signal });
