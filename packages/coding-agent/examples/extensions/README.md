...
Example extensions for AOS Agent.

## Usage

```bash
# Load an extension with --extension flag
aos --extension examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.aos-agent/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `agent.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs AOS Agent theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `agent.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider Using AOS Agent-ai's built-in Anthropic/OpenAI streaming via proxy |
