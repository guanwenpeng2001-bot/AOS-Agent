...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "aos-agent";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "@aos-agent/tui";
...
export default function (agent: ExtensionAPI) {
	agent.on("session_start", async (_event, ctx) => reconstructState(ctx));
	agent.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	agent.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	agent.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	agent.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			agent.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						agent.appendEntry(SAVE_TYPE, getBoardDetails());
