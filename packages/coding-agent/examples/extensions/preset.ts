 ...
 * - ~/.aos-agent/agent/presets.json (global)
 * - <cwd>/.aos-agent/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `aos --preset plan` - start with plan preset
...
import { CONFIG_DIR_NAME, DynamicBorder, getAgentDir } from "aos-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@aos-agent/tui";
...
export default function presetExtension(agent: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: agent.getThinkingLevel(),
		...
		if (preset.thinkingLevel) {
			agent.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = agent.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				agent.setActiveTools(validTools);
			}
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await agent.setModel(originalState.model);
				}
				agent.setThinkingLevel(originalState.thinkingLevel);
				agent.setActiveTools(originalState.tools);
			} else {
				agent.setActiveTools(["read", "bash", "edit", "write"]);
			}
	...
	agent.registerShortcut(Key.ctrlShift("u"), {
	...
	agent.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = agent.getFlag("preset");
	...
	agent.on("turn_start", async () => {
		if (activePresetName) {
			agent.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
