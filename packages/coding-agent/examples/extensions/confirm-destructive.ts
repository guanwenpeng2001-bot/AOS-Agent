...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "aos-agent";

export default function (agent: ExtensionAPI) {
	agent.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
