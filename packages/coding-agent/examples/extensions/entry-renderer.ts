 ...
 * to the LLM. Custom entries are stored in the session via agent.appendEntry() and
 * rendered in interactive mode via agent.registerEntryRenderer().
 *
...
export default function (agent: ExtensionAPI) {
	agent.registerEntryRenderer<StatusCardData>("status-card", (entry, { expanded }, theme) => {
	...
	agent.registerCommand("status-card", {
		description: "Render a durable status card that is not sent to the LLM",
		handler: async (args) => {
			agent.appendEntry<StatusCardData>("status-card", {
