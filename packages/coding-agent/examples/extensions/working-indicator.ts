 ...
 * working indicator shown while AOS Agent is streaming a response.
 *
 * Usage:
 *   aos --extension examples/extensions/working-indicator.ts
 *
 * Commands:
 ...
 *   /working-indicator reset     Restore AOS Agent's default spinner
 */

import type { ExtensionAPI, ExtensionContext, WorkingIndicatorOptions } from "aos-agent";
...
function describeMode(mode: WorkingIndicatorMode): string {
	switch (mode) {
		...
		case "default":
			return "AOS Agent default spinner";
	}
}
...
export default function (agent: ExtensionAPI) {
	...
	agent.registerCommand("working-indicator", {
