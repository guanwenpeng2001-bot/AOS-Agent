 ...
 *
 * Demonstrates agent.sendUserMessage() for sending user messages from extensions.
 * Unlike agent.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (agent: ExtensionAPI) {
	...
	agent.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			agent.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	agent.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				agent.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	agent.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				agent.sendUserMessage(args, { deliverAs: "followUp" });
