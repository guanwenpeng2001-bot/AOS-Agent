> AOS Agent can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend AOS Agent's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.aos-agent/agent/extensions/` (global) or `.aos-agent/extensions/` (project-local) for auto-discovery. Use `aos -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.

**Key capabilities:**
- **Custom tools** - Register tools the LLM can call via `agent.registerTool()`
- **Event interception** - Block or modify tool calls, inject context, customize compaction
- **User interaction** - Prompt users via `ctx.ui` (select, confirm, input, notify)
- **Custom UI components** - Full TUI components with keyboard input via `ctx.ui.custom()` for complex interactions
- **Custom commands** - Register commands like `/mycommand` via `agent.registerCommand()`
- **Session persistence** - Store state that survives restarts via `agent.appendEntry()`
...
Create `~/.aos-agent/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.aos-agent/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.aos-agent/agent/extensions/*.ts` | Global (all projects) |
| `~/.aos-agent/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.aos-agent/extensions/*.ts` | Project-local |
| `.aos-agent/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as aos-agent packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `aos-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@aos-agent/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@aos-agent/tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed aos-agent packages installed with `aos install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
export default function (agent: ExtensionAPI) {
  // Subscribe to events
  agent.on("event_name", async (event, ctx) => {
...
If the factory returns a `Promise`, AOS Agent awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `agent.registerProvider()` are flushed.
...
export default async function (agent: ExtensionAPI) {
  ...
  agent.registerProvider("local-openai", {
...
This pattern makes the fetched models available during normal startup and to `aos --list-models`.
...
thinking level changes (settings, keybinding, agent.setThinkingLevel())
...
```typescript
agent.on("resources_discover", async (event, _ctx) => {
  // event.cwd - current working directory
...
Fired when the current session display name is set via `/name`, RPC, or `agent.setSessionName()`.

```typescript
agent.on("session_info_changed", async (event, ctx) => {
...
After a successful switch or new-session action, AOS Agent emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
```typescript
agent.on("session_before_fork", async (event, ctx) => {
...
After a successful fork or clone, AOS Agent emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
```typescript
agent.on("session_before_tree", async (event, ctx) => {
...
agent.on("session_tree", async (event, ctx) => {
...
```typescript
agent.on("session_shutdown", async (event, ctx) => {
...
`agent_start` fires when a low-level agent run begins. `agent_end` fires when that run ends, but AOS Agent may still auto-retry, auto-compact and retry, or continue with queued follow-up messages. Use `agent_settled` for status integrations that need to know AOS Agent will not continue running automatically.

```typescript
agent.on("agent_start", async (_event, ctx) => {});

agent.on("agent_end", async (event, ctx) => {
  // event.messages - messages from this low-level run
});

agent.on("agent_settled", async (_event, ctx) => {
...
```typescript
agent.on("turn_start", async (event, ctx) => {
  // event.turnIndex, event.timestamp
});

agent.on("turn_end", async (event, ctx) => {
...
```typescript
agent.on("message_start", async (event, ctx) => {
  // event.message
});

agent.on("message_update", async (event, ctx) => {
  // event.message
  // event.assistantMessageEvent (token-by-token stream event)
});

agent.on("message_end", async (event, ctx) => {
...
```typescript
agent.on("tool_execution_start", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args
});

agent.on("tool_execution_update", async (event, ctx) => {
  // event.toolCallId, event.toolName, event.args, event.partialResult
});

agent.on("tool_execution_end", async (event, ctx) => {
...
```typescript
agent.on("before_provider_headers", (event, ctx) => {
  // Add or override — e.g. a session id for gateway tracing/attribution
  event.headers["x-session-id"] = ctx.sessionManager.getSessionId();

  // Drop a tracking header AOS Agent adds for this call
...
```typescript
agent.on("model_select", async (event, ctx) => {
...
```typescript
agent.on("thinking_level_select", async (event, ctx) => {
...
Use this to update extension UI when `agent.setThinkingLevel()`, model changes, or built-in thinking-level controls change the active thinking level.
...
Before `tool_call` runs, AOS Agent waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "aos-agent";
...
```typescript
import { isBashToolResult } from "aos-agent";

agent.on("tool_result", async (event, ctx) => {
...
```typescript
import { createLocalBashOperations } from "aos-agent";

agent.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap AOS Agent's built-in local bash backend
...
```typescript
agent.on("input", async (event, ctx) => {
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.aos-agent` when constructing project-local config paths. Rebranded distributions can use a different config directory name.
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while AOS Agent is idle.
...
Control flow helpers. `ctx.isIdle()` is false while AOS Agent is processing an agent run, automatic retry, auto-compaction retry, or queued continuation.

### ctx.shutdown()

Request a graceful shutdown of AOS Agent.
...
Returns the base inputs AOS Agent currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `agent` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "aos-agent";

agent.registerCommand("switch", {
...
- Captured old `agent` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
agent.registerCommand("reload-runtime", {
...
export default function (agent: ExtensionAPI) {
  ...
  agent.registerTool({
    ...
    parameters: Type.Object({}),
    async execute() {
      agent.sendUserMessage("/reload-runtime", { deliverAs: "followUp" });
      return {
...
`agent.registerTool()` works both during extension load and after startup. You can call it inside `session_start`, command handlers, or other event handlers. New tools are refreshed immediately in the same session, so they appear in `agent.getAllTools()` and are callable by the LLM without `/reload`.

Use `agent.setActiveTools()` to enable or disable tools (including dynamically added tools) at runtime.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@aos-agent/ai";
...
Inject a custom message into the session. Custom messages participate in LLM context. For durable TUI-only content that should not be sent to the LLM, use [`agent.appendEntry()`](#piappendentrycustomtype-data) with [`agent.registerEntryRenderer()`](#piregisterentryrenderercustomtype-renderer).
...
### agent.sendUserMessage(content, options?)

Send a user message to the agent. Unlike `sendMessage()` which sends custom messages, this sends an actual user message that appears as if typed by the user. Always triggers a turn.

```typescript
// Simple text message
agent.sendUserMessage("What is 2+2?");
...
agent.sendUserMessage("Focus on error handling", { deliverAs: "steer" });
agent.sendUserMessage("And then summarize", { deliverAs: "followUp" });
```
...
### agent.appendEntry(customType, data?)

Persist extension data. Custom entries do NOT participate in LLM context. In interactive mode, they can also render inside the chat transcript when paired with `agent.registerEntryRenderer()`.

```typescript
agent.appendEntry("my-state", { count: 42 });
agent.appendEntry("status-card", { title: "Indexed files", count: 17 });
...
```typescript
agent.setSessionName("Refactor auth module");
```
...
```typescript
const name = agent.getSessionName();
if (name) {
  console.log(`Session: ${name}`);
}
```

### agent.setLabel(entryId, label)

Set or clear a label on an entry. Labels are user-defined markers for bookmarking and navigation (shown in `/tree` selector).

```typescript
// Set a label
agent.setLabel(entryId, "checkpoint-before-refactor");

// Clear a label
agent.setLabel(entryId, undefined);
...
### agent.registerCommand(name, options)

Register a command.

If multiple extensions register the same command name, AOS Agent keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.

```typescript
agent.registerCommand("stats", {
...
```typescript
import type { AutocompleteItem } from "@aos-agent/tui";
...
### agent.registerMessageRenderer(customType, renderer)

Register a custom TUI renderer for custom messages with your `customType`. Custom messages are created with `agent.sendMessage()` and participate in LLM context. See [Custom UI](#custom-ui).

### agent.registerMarkdownTransformer(transformer)

Register a transformer for the Markdown in normal user text, assistant text, and thinking blocks. Transformers run in extension load order, and each transformer receives the Markdown returned by the previous transformer. After the chain finishes, AOS Agent renders the transformed content with its built-in renderer.
...
```typescript
agent.registerMarkdownTransformer((markdown, { messageType, isStreaming }) => {
...
### agent.registerEntryRenderer(customType, renderer)

Register a custom TUI renderer for custom entries with your `customType`. Custom entries are created with `agent.appendEntry()` and do not participate in LLM context.

```typescript
import { Box, Text } from "@aos-agent/tui";

agent.registerEntryRenderer("status-card", (entry, { expanded }, theme) => {
...
### agent.registerShortcut(shortcut, options)

Register a keyboard shortcut. See [keybindings.md](keybindings.md) for the shortcut format and built-in keybindings.

```typescript
agent.registerShortcut("ctrl+shift+p", {
...
### agent.registerFlag(name, options)

Register a CLI flag.

```typescript
agent.registerFlag("plan", {
...
// Check value
if (agent.getFlag("plan")) {
  // Plan mode enabled
}
```

### agent.exec(command, args, options?)

Execute a shell command.

```typescript
const result = await agent.exec("git", ["status"], { signal, timeout: 5000 });
// result.stdout, result.stderr, result.code, result.killed
```

### agent.getActiveTools() / agent.getAllTools() / agent.setActiveTools(names)

Manage active tools. This works for both built-in tools and dynamically registered tools. `agent.getActiveTools()` returns the active tool names as `string[]`; `agent.getAllTools()` returns metadata for all configured tools.

```typescript
const active = agent.getActiveTools(); // ["read", "bash", ...]
...
agent.setActiveTools([...new Set([...active, "my_custom_tool"])]); // Keep current tools and enable my_custom_tool
agent.setActiveTools(["read", "bash"]); // Switch to read-only
```

`agent.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
if (model) {
  const success = await agent.setModel(model);
  if (!success) {
...
### agent.getThinkingLevel() / agent.setThinkingLevel(level)
...
```typescript
const current = agent.getThinkingLevel();  // "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@aos-agent/ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@aos-agent/ai";
...
agent.registerProvider("my-proxy", {
  name: "My Proxy",
...
agent.registerProvider("llama.cpp", {
  baseUrl: "http://localhost:8080/v1",
  apiKey: "local",
  api: "openai-completions",
...
agent.registerProvider("corporate-ai", {
...
The object form accepts a complete @aos-agent/ai `Provider`, including native `auth`, `getModels`, `refreshModels`, `filterModels`, `stream`, and `streamSimple` behavior.
...
```typescript
agent.registerCommand("my-setup-teardown", {
  description: "Remove the custom proxy provider",
  handler: async (_args, _ctx) => {
    agent.unregisterProvider("my-proxy");
  },
});
```
...
Register tools the LLM can call via `agent.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `agent.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "aos-agent";
...
agent.registerTool({
  ...
  async execute(toolCallId, params, signal, onUpdate, ctx) {
    ...
    // Run commands via agent.exec (captured from extension closure)
    const result = await agent.exec("some-command", [], { signal });
...
**Usage accounting:** If a tool makes nested LLM calls, return their combined `Usage` as `usage`. AOS Agent persists it on the tool result and includes it in footer, `/session`, and RPC session totals. `tool_result` handlers can inspect or replace this value.
...
**Important:** Use `StringEnum` from `@aos-agent/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when AOS Agent resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
aos -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
aos --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](UPSTREAM.md) - `ReadToolDetails`
- [bash.ts](UPSTREAM.md) - `BashToolDetails`
- [edit.ts](UPSTREAM.md)
- [write.ts](UPSTREAM.md)
- [grep.ts](UPSTREAM.md) - `GrepToolDetails`
- [find.ts](UPSTREAM.md) - `FindToolDetails`
- [ls.ts](UPSTREAM.md) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "aos-agent";
...
For `user_bash`, extensions can reuse AOS Agent's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.

The bash tool also supports a spawn hook to adjust the command, cwd, or env before execution:

```typescript
import { createBashTool } from "aos-agent";
...
`createBashTool()` exposes the current session to commands through `AOS_AGENT_SESSION_ID`, `AOS_AGENT_SESSION_FILE`, `AOS_AGENT_PROVIDER`, `AOS_AGENT_MODEL`, and `AOS_AGENT_REASONING_LEVEL`. Injection happens before `spawnHook`, so hooks receive these values in `env` and preserve them when they spread the existing environment as above. Set `exposeSessionEnvironment: false` to disable them:
...
export default function (agent: ExtensionAPI) {
  ...
  agent.registerTool({ name: "db_connect", ... });
  agent.registerTool({ name: "db_query", ... });
  agent.registerTool({ name: "db_close", ... });
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](UPSTREAM.md) for how tool rows are composed.
...
1. Register every tool with `agent.registerTool()` so it appears in `agent.getAllTools()`.
2. Keep loader tools, such as `search_tools`, active and leave searchable tools inactive.
3. During loader execution, call `agent.setActiveTools([...currentTools, ...matchingTools])`. The change must be additive: do not remove currently active tools in the same call.
4. AOS Agent records which tools were added on the loader's tool result.
5. Before the next model response, AOS Agent exposes the added definitions using native deferred loading when supported, or the normal active tool list otherwise.

You do not need to return provider-specific tool references or mark the loader as a special search tool. The active-tool change is the signal. Names passed to `agent.setActiveTools()` must already be registered; unknown names are ignored.
...
- **OpenAI**
  ...
  - **Native representation:** AOS Agent adds completed client `tool_search_call` and `tool_search_output` items at the load point.
...
For all other models and providers, dynamic activation still works: AOS Agent sends the complete current active tool list normally on the next request. The model can call the newly activated tools, but adding their definitions may invalidate the provider's cached prompt prefix.

AOS Agent also uses this safe fallback when the active set is not purely additive, such as replacing one group of tools with another. Tool removals therefore work, but they do not use deferred loading.
...
export default function (agent: ExtensionAPI) {
  ...
  agent.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      const matches = agent.getAllTools()
      ...
      const active = agent.getActiveTools();
      const added = matches.filter((name) => !active.includes(name));
      agent.setActiveTools([...new Set([...active, ...added])]);
  ...
  agent.on("session_start", () => {
    ...
    const initialTools = agent.getActiveTools().filter(
      (name) => !SEARCHABLE_TOOL_NAMES.has(name),
    );
    agent.setActiveTools([...new Set([...initialTools, "search_tools"])]);
  });
}
```
...
```typescript
// Select from options
const choice = await ctx.ui.select("AOS Agentck one:", ["A", "B", "C"]);
...
// Terminal title
ctx.ui.setTitle("aos - my-project");
...
```typescript
import { Text, Component } from "@aos-agent/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "aos-agent";
import { matchesKey } from "@aos-agent/tui";
...
agent.registerMessageRenderer("my-extension", (message, options, theme) => {
...
Messages are sent via `agent.sendMessage()`:
...
```typescript
agent.registerEntryRenderer("my-card", (entry, options, theme) => {
  return new Text(theme.fg("accent", JSON.stringify(entry.data)));
});

agent.appendEntry("my-card", { status: "done" });
```
...
```typescript
import { highlightCode, getLanguageFromPath } from "aos-agent";
...
| `event-bus.ts` | Inter-extension events | `agent.events` |
| **Session Metadata** |||
