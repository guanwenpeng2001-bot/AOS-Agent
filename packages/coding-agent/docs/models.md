  ...
  "modelBroker": {
    ...
    "routes": {
      "balanced": {
        ...
        "budget": { "maxModelCalls": 12, "maxTotalTokens": 300000, "maxCostUsd": 2 }
      }
    },
...
other authentication material. Budget limits apply to the top-level prompt
operation (or the Automation Run); a response that overruns a limit is kept,
but later model calls are blocked. Use `/model-routes` to inspect the redacted
...
The `apiKey` value is a placeholder because Ollama ignores it. AOS Agent still treats models as requiring auth before they appear in `/model`, so keyless local servers should keep a dummy value, save a key for that provider with `/login`, or pass `--api-key` when selecting the model.
...
For `models.json`, shell commands are resolved at request time. AOS Agent intentionally does not apply built-in TTL, stale reuse, or recovery logic for arbitrary commands. Different commands need different caching and failure strategies, and AOS Agent cannot infer the right one.
...
| `thinkingLevelMap` | No | omitted | Maps AOS Agent thinking levels to provider values and marks unsupported levels (see below) |
...
`samplingParams` is a free-form object merged verbatim into every request body for the model, after the fields AOS Agent sets itself, so its keys win. Use it to send sampling parameters AOS Agent does not model — including server-specific ones like llama.cpp's `min_p` or vLLM's `top_k`:
...
Only OpenAI-compatible APIs apply it (`openai-completions`, `openai-responses`, `azure-openai-responses`); other APIs ignore it. Keys override AOS Agent's named request fields (for example a `temperature` key here beats the request-level temperature), so prefer it as the single source of sampling truth for a model. In `modelOverrides`, `samplingParams` merges per key with the base model's value.

### Thinking Level Map

Use `thinkingLevelMap` on a model to describe model-specific thinking controls. Keys are AOS Agent thinking levels: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`. Maps may contain holes; for example, a model can expose `high` and `max` without exposing `xhigh`.
...
| `supportsFinishReason` | Whether streamed responses include `finish_reason`. When `false`, AOS Agent infers `stop` or `toolUse` when the stream ends. Default: `true`. |
...
| `chatTemplateKwargs` | `chat_template_kwargs` values for `thinkingFormat: "chat-template"`; use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for AOS Agent-controlled thinking values |
| `chatTemplateArgs` | `chat_template_args` values for `thinkingFormat: "baseten"`; use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for AOS Agent-controlled thinking values |
