...
An isolated root `./test.sh` run was also attempted. Its remaining failures are
outside the T12 diff: the pre-existing offline AI catalog baseline, child
process imports that require workspace `dist` output, and WSL platform/permission
assumptions. This audit does not report that root gate as passing.
