...
```
~/.aos-agent/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
A clean close retains the last fence and marks its lease released, so the next
writer advances rather than resetting that generation. A crashed Host retains
its positive deadline. Once it expires, a new Host must still explicitly call
`takeOver`; ordinary writer open never performs automatic failover. The fence
advance and its `writer_takeovers` audit row commit atomically. Audit rows retain
the previous/current Host ids, previous/current fences, prior deadline,
timestamp, and `forced` or `expired` reason.
...
AOS Agent also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, AOS Agent uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/aos-agent/dist/` and `node_modules/@aos-agent/ai/dist/`.
...
### Base Message Types (from @aos-agent/ai)
...
The exported @aos-agent/ai `StopReason` type also includes `"pending"`, but that value is reserved for partial messages in streaming events. Terminal `done`/`error` messages replace it with a completion reason before AOS Agent persists the assistant message, so `"pending"` should never appear in session JSONL.

### Extended Message Types (from aos-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if AOS Agent-generated (legacy field name)
...
Use `customType` to identify your extension's entries on reload. Interactive mode can render custom entries via `agent.registerEntryRenderer(customType, renderer)`, but they still do not participate in LLM context.
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `agent.setSessionName()` in extensions.
