...
AOS Agent runs with all permissions by default, but in some cases, you will want to have more control over what directories AOS Agent can write to and which accesses it has.

There are two general options. You can either
1. run the whole `aos` process inside an isolated environment, or
2. run `aos` on the host and route tool execution into an isolated environment.
...
Use the [example extension](../examples/extensions/gondolin) when you want `aos` on the host but all built-in tools routed into the VM.
...
Use a named volume for `/root/.aos-agent/agent` if you want container-local settings and sessions. Mounting your host `~/.aos-agent/agent` exposes host auth and session files to the container.
...
In this pattern, the whole `aos` process runs inside the sandbox.
...
Configure AOS Agent to use the corresponding OpenAI-compatible or Anthropic-compatible endpoint if you want model traffic to use this route.
