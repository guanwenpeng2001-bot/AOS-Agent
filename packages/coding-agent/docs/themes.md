> AOS Agent can create themes. Ask it to build one for your setup.
...
AOS Agent loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.aos-agent/agent/themes/*.json`
- Project: `.aos-agent/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `aosAgent.themes` entries in `package.json`
...
On first run, AOS Agent detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.aos-agent/agent/themes
vim ~/.aos-agent/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/AOS Agent/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, AOS Agent reloads it automatically for immediate visual feedback.
...
AOS Agent uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, AOS Agent falls back to the nearest approximation.
