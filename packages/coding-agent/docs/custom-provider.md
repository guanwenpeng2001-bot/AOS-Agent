...
Extensions can register custom model providers via `agent.registerProvider()`. This enables:
...
Extensions can register either a complete @aos-agent/ai `Provider` or use the legacy provider-config form. Prefer a complete provider when custom authentication, filtering, refresh, or streaming behavior is required. AOS Agent composes `models.json` overrides above registered native providers.
...
export default function (agent: ExtensionAPI) {
  agent.registerProvider(createProvider({
    id: "native-local",
    name: "Native Local",
  ...
  agent.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. AOS Agent waits for the factory before startup continues, so the provider is available during interactive startup and to `aos --list-models`.
...
Use `agent.unregisterProvider(name)` to remove a provider that was previously registered via `agent.registerProvider(name, ...)`:
...
// Later, remove it
agent.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map AOS Agent levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
agent.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@aos-agent/ai";
...
Credentials are persisted in `~/.aos-agent/agent/auth.json`:
...
When a request exceeds the model's context window, AOS Agent can recover automatically by compacting the conversation and retrying. This recovery only kicks in if AOS Agent recognizes the failure as an overflow.
...
export default function (agent: ExtensionAPI) {
  ...
  agent.on("message_end", (event, ctx) => {
...
`message_end` runs before AOS Agent tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what AOS Agent checks. With this in place, AOS Agent will:
...
- Match a provider-specific pattern, not AOS Agent's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of AOS Agent's normal retry-with-backoff path.
...
interface ProviderModelConfig {
  ...
  /** Maps AOS Agent thinking levels to provider/model-specific values; null marks a level unsupported. */
