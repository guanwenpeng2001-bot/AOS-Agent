...
`RpcClient` owns the Host transport below this server. Both handlers depend only on typed client methods, so the Web server does not access Session internals or control-plane stores directly. The page polls every three seconds; it does not subscribe to a live event stream. The write boundary is intended for one local operator; it does not add authentication or remote access.
