> AOS Agent can create TUI components. Ask it to build one for your use case.
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@aos-agent/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `AOS_AGENT_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@aos-agent/tui";
...
Import from `@aos-agent/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@aos-agent/tui";
```
...
```typescript
import { matchesKey, Key } from "@aos-agent/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@aos-agent/tui";
...
```typescript
import { getMarkdownTheme } from "aos-agent";
import { Markdown } from "@aos-agent/tui";
...
Set `AOS_AGENT_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
AOS_AGENT_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@aos-agent/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "aos-agent";
import { DynamicBorder } from "aos-agent";
import { Container, type SelectItem, SelectList, Text } from "@aos-agent/tui";
...
```typescript
import { BorderedLoader } from "aos-agent";

agent.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@aos-agent/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "aos-agent";
import { Container, type SettingItem, SettingsList, Text } from "@aos-agent/tui";

agent.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while AOS Agent is streaming a response.
...
// Restore AOS Agent's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@aos-agent/tui";
