> AOS Agent can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to AOS Agent's agent capabilities. Use it to embed AOS Agent in other applications, build custom interfaces, or integrate with automated workflows.
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `agent.sendMessage()`.
...
The `Agent` class (from `@aos-agent/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.aos-agent/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.aos-agent/extensions/`)
- Project skills:
  - `.aos-agent/skills/`
...
import { ModelRuntime } from "aos-agent";
...
```typescript
import { InMemoryCredentialStore } from "@aos-agent/ai";
import { createAgentSession, ModelRuntime } from "aos-agent";

// Default: uses ~/.aos-agent/agent/auth.json and ~/.aos-agent/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
// Or inject any @aos-agent/ai CredentialStore
...
The `edit` tool returns `details.diff` for AOS Agent's TUI display and `details.patch` as a standard unified patch for SDK consumers.
...
import { createAgentSession, defineTool } from "aos-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `agent.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `agent.registerTool()`.
...
```typescript
import type { InlineExtension } from "aos-agent";
...
**Event Bus:** Extensions can communicate via `agent.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "aos-agent";
...
1. Global: `~/.aos-agent/agent/settings.json`
2. Project: `<cwd>/.aos-agent/settings.json`
