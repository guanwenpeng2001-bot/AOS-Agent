...
Vendor-neutral telemetry contracts and typed schema utilities for AOS Agent packages.
...
- [AOS Agent Package Integration](#aos-agent-package-integration)
...
```bash
npm install @aos-agent/telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@aos-agent/telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@aos-agent/telemetry';
...
`@aos-agent/telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@aos-agent/telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@aos-agent/agent-core';
...
## AOS Agent Package Integration

Package ownership is intentionally split:

- `@aos-agent/telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@aos-agent/ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@aos-agent/agent-core` owns and exports the AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
