...
```typescript
import { type TUI, Text, Editor, ProcessTerminal, TuiMainScreen, matchesKey } from "@aos-agent/tui";
...
```typescript
import { type TUI, TuiAltScreen, TuiMainScreen } from "@aos-agent/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the renderer constructor's `showHardwareCursor` argument, `setShowHardwareCursor(true)`, or `AOS_AGENT_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { CombinedAutocompleteProvider } from "@aos-agent/tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@aos-agent/tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@aos-agent/tui";
...
```bash
AOS_AGENT_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
