...
export const VIEWPORT_TUI = Symbol.for("@aos-agent/tui/viewport");
...
export abstract class TuiBase extends Container implements TUI {
	...
	private showHardwareCursor = process.env.AOS_AGENT_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.AOS_AGENT_CLEAR_ON_SHRINK === "1";
	...
	constructor(terminal: Terminal, showHardwareCursor?: boolean, logDirectory?: string) {
		...
		this.logDirectory = logDirectory ?? process.env.AOS_AGENT_DIR ?? process.env.AOS_AGENT_CODING_AGENT_DIR ?? path.join(os.homedir(), ".aos-agent", "agent");
