...
AOS Agent and also where the boundaries are.

In general AOS Agent is a coding agent that runs locally within the security boundary
...
AOS Agent treats the local user account and files writable by that account as inside
the same trust boundary as the AOS Agent process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
AOS Agent configuration, they can generally influence AOS Agent or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how AOS Agent grants that write access or crosses
an operating-system privilege boundary.

AOS Agent relies on users installing trustworthy extensions and loading trustworthy
skills and only to use AOS Agent within trusted repositories.  This is because files
...
- Local code execution or sandboxing behavior (the AOS Agent coding agent intentionally does not have a sandbox)
- Behavior of AOS Agent extensions or skills installed by the user
...
- Public internet exposure of an AOS Agent installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.aos-agent`,
  `~/.aos-agent/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how AOS Agent itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against the AOS Agent coding agent.
...
For example, a report showing that malicious contents written to a trusted AOS Agent
configuration file cause AOS Agent to execute commands, load attacker-controlled tools,
