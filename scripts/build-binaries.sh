#!/usr/bin/env bash
#
# Build aos binaries for all platforms locally.
#
# Usage:
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        ...
        mv "$platform" aos && tar -czf aos-$platform.tar.gz aos && mv aos "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/aos.exe"
    else
        echo "  $OUTPUT_DIR/$platform/aos"
    fi
done
