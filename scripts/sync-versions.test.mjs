...
test("synchronizes private dependencies without touching registry aliases, generated manifests, or published lockstep", async () => {
	const root = await mkdtemp(join(tmpdir(), "aos-agent-sync-versions-"));
	try {
		...
		await writeManifest(root, "packages/evals", {
			...
			dependencies: {
				"aos-agent": "^1.0.0",
				"@aos-agent/ai": "npm:@aos-agent/ai@1.0.0",
			},
		});
		...
		assert.equal(generatedManifest.dependencies["aos-agent"], "^1.0.0");
