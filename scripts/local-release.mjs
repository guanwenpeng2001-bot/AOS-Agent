...
const packages = [
	{ directory: "packages/telemetry", name: "@aos-agent/telemetry" },
	{ directory: "packages/ai", name: "@aos-agent/ai" },
	{ directory: "packages/tui", name: "@aos-agent/tui" },
	{ directory: "packages/agent", name: "@aos-agent/agent-core" },
	{ directory: "packages/protocol", name: "@aos-agent/protocol" },
	{ directory: "packages/client", name: "@aos-agent/client" },
	{ directory: "packages/session-backends/sqlite-node", name: "@aos-agent/session-backend-sqlite-node" },
	{ directory: "packages/server", name: "@aos-agent/server" },
	{ directory: "packages/coding-agent", name: "aos-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "aos-agent-local-release-"));
	}
...
function createAosShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "aos.cmd"))) {
			writeFileSync(join(installDirectory, "aos.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\aos.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "aos.ps1"), '& "$PSScriptRoot/node_modules/.bin/aos.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "aos.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\aos.exe" %*\r\n');
		writeFileSync(join(installDirectory, "aos.ps1"), '& "$PSScriptRoot/node_modules/.bin/aos.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "aos"), join(installDirectory, "aos"));
}
...
if (rootPackageJson.name !== "aos-agent-source") {
...
if (!options.skipInstall) {
	...
	createAosShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createAosShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `aos-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "aos.exe" : "aos")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "aos.cmd" : "aos")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "aos.cmd" : "aos")} --help`);
	}
}
