 ...
 * 10. Push main and the tag to trigger host CI publication
 *     (upstream aos-agent.example.invalid announcement is disabled for AOS Agent)
 */
...
function bumpOrSetVersion(target) {
	...
	if (BUMP_TYPES.has(target)) {
		...
		run(`npm version ${target} --workspaces --no-git-tag-version --no-workspaces-update && node scripts/sync-versions.mjs && npm install --package-lock-only --ignore-scripts`);
	}
...
console.log(`=== Prepared release v${version}; host CI publication starts after the tag push (aos-agent.example.invalid announcement disabled) ===`);
